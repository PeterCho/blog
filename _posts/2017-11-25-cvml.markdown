...
We empirically found the following combinations of convolutions with 5x5
kernels, batch normalization, leaky ReLU activation [8], 2x2 pooling and
fully connected layers to be useful:

CBLP: Conv, Batch Norm, Leaky Relu, Pool

CBL: Conv, Batch Norm, Leaky Relu

BLD: Batch Norm, Leaky Relu, Dense Conv with no bottleneck

FBL: Fully connected, Batch Norm, Leaky Relu

We choose these super-layers' parameters and their number within network
configurations so that total receptive field and training image pixel sizes
are comparable.  Our top three CNN architectures all have receptive field
size R=157 [9]:

Architecture I with 9 super-layers: 
    3 x [CBLP], 4 x [CBL], 2 x [FBL]

Architecture II with 10 super-layers: 
    2 x [CBLP], 2 x [CBL], CBLP, 3 x [CBL], 2 x [FBL]

Architecture III with 10 super-layers including 5 dense blocks:
    2 x [CBLP], 2 x [BLD], CBLP, 3 x [BLD], 2 x [FBL]
...
*Table 1.  List of common CNN hyperparameters.*
...
![CatsDogs]({{site.url}}/blog/images/cats_and_dogs/multiexpt_preferred_final_ocnv_filters.png)
*Fig. 8.  Examples of preferred inputs for trained filters within final 
convolution layers of CNNs with architectures I, II and III.
