...
winner, Zhao Xing, attained 98.3% accuracy.
...
kernels, batch normalization, leaky ReLU activation [8], 2x2 pooling and
fully connected layers to be useful building blocks:
...
Architecture I with 9 nonlinear super-layers: 

$\qquad$ 3 $\times$ [CBLP], 4 $\times$ [CBL], 2 $\times$ [FBL], SOFT

Architecture II with 10 nonlinear super-layers: 
...
$\qquad$ 2$\times$ [CBLP], 2$\times$ [BLD], CBLP, 3$\times$ [BLD],
2$\times$ [FBL], SOFT
...
continuous.  Table 1 lists hyperparameter values which we found worked well
with CNN architectures I, II and III for classifying cats and dogs.  It is
worth noting that this table does not include any dropout probability or
weight decay coefficient.  We observed that batch normalization plus data
...
The classifier is consequently exposed to increasingly difficult training
samples as the CNN learns.

Learning curves for one representative training session are plotted in
figure 4.  Fractional classification accuracies on both the training and
...
challenging than their validation sample counterparts.  So the green
learning curve for the training subset in figure 4 increases more slowly
than the purple curve for the validation data.  Yet by the end of the
experiment, both reach similar classification accuracy fractions.  We
consequently do not observe signs of significant over-fitting or
under-fitting in this plot or others like it for our three CNN
architectures.
...
data sets for one particular experiment training an architecture I CNN.*
...
cumulative score.  This aggregation approach effectively corresponds to a
weighted voting scheme for binary classification.

Inference performed using an ensemble of classifiers generally yields
better performance than any of its individual constituents [10].  We apply
ensemble classification on the Kaggle test set of 9970 cat and dog images
which we manually labeled.  The ensemble of 8 trained CNNs achieves a
classification accuracy of 98.0% on the held-out test set.  While our
classifier's performance does not exceed the 2014 competition winner's, we
intentionally worked with downsampled versions of the Kaggle imagery for
speed purposes and did not use any other external data.
...
sometimes contain occluding foreground objects such as cages.  It is
interesting to compare examples of correctly classified test images with
those in figure 6 representing incorrect classification results.  In some
cases, wrongly classified animals appear small in the image due to physical
size or distance from the camera.  In others, the animals' orientations are
uncommon.  Yet after having looked through the O(200) of O(10K) test images
which were incorrectly classified by the CNN ensemble, we have not observed
obvious signals which differentiate them from correctly classified photos.
...
To gain insight into how trained CNNs learn to distinguish cats from dogs,
we need to investigate more results than just test imagery classification
results.  We take up this topic in the next section.

## Visualizing CNN Learning

Neural network feature visualization sheds light upon what CNN neurons
learn to recognize [11].  Starting from some node within a trained network,
we can backpropagate to an input image initialized with noise.  The image's
values are optimized to maximally activate the neuron while all trained
network weights are held fixed.  Details within views of inputs preferred
by trained filters vary each time they are synthesized from different
random noise.  But the qualitative appearance of preferred inputs for
entire CNN layers is generally stable.

Figure 7 displays representative examples of inputs which maximally
activate trained neurons within each of the convolutional layers in one of
our architecture I CNNs.  These results were generated via a minor variant
of TensorFlow code developed by Alex Mordvintsev [12].  Nodes in the
network's first convolutional layer respond to input image colors.  Neurons
in deeper layers fire when they observe increasingly complicated textures
and patterns.  Zooming into the sixth layer of figure 7, we see filter
responding to cat ears and dog eyes.  Entire animal faces become
recognizable within the preferred inputs for some nodes in the final layer
of the trained CNN.
...
It is interesting to compare feature visualization results for different
networks trained on the same imagery corpus.  Figure 8 presents such a
comparison among four of the eight CNNs within our ensemble used to
classify cat and dog test images.  The final layer results in this figure
were intentionally selected based upon their cat and dog face contents.
Qualitatively different animal faces can be recognized within the preferred
inputs of final convolution layer nodes among the different trained CNNs.
Such qualitative variation among filter responses holds even for model
snapshots from a single CNN at different epochs during its training.  It is
consequently not surprising that the classification performance from an
ensemble of trained CNNs is superior to that of any of its individual
members.
...
Our final visualization of CNN learning focuses upon class clustering as
training proceeds.  Nonlinear layers in CNNs generally transform input
...
\times$ 148 images into 12-dimensional descriptors which are fed into its
final softmax classifier.  Before training starts, the CNN's weights are
randomly initialized, and cat plus dog images are intermingled within the
12-dimensional descriptor space.  But as the CNN's weights evolve during
training, we would expect distinct clusters of cat and dog descriptors to
develop.  Visualizing the formation of such class clusters provides an
instructive view into CNN learning.

The movie in figure 9 illustrates PCA decompositions of image descriptors
from the final fully connected nonlinear layer of CNN architecture I.
Working with a fully trained network, we calculcated the PCA
eigendirections from a distribution of O(2.4K) validation image
descriptors.  12-dimensional validation descriptors were subsequently
projected onto the first three dominant PCA eigendirections at training
epochs 0, 4, 8 and 16.  The red and blue dots appearing within the figure
represent projected cat and dog descriptors.  As the movie plays, we
clearly observe more separation with increasing training epoch between
projected cat and dog descriptors within the 3D point clouds.  The
relatively small overlap between cat and dog points within the last
quadrant of figure 9 would presumably diminish if more PCA eigendirections
could be plotted.  But some intermingling among descriptors would survive
since CNN classification is never perfect.
