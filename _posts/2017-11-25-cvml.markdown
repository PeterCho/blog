...
selected neuron while all trained network weights are held fixed.  The
detailed appearances of input views preferred by trained filters vary each
time they are synthesized from random noise.  But the qualitative pattern
of inputs preferred by entire layers in a CNN is generally stable.
...
different training epochs.  It consequently makes sense that classification
performance from an ensemble of trained CNNs is superior to that of any
individual member.
...
Our final visualization of CNN learning focuses upon binary class
clustering.  Nonlinear layers in CNNs generally transform input images into
representations which are easier for their final classifier layer to
handle.  For example, our architecture I CNN converts 148 $ \times$ 148
images into 12-dimensional descriptors which feed into its final softmax
classifier.  Before training starts, the CNN's weights are randomly
initialized.  Descriptors corresponding to cat and dog input images are
then intermingled within the 12-dimensional output space.  But as the CNN's
weights evolve during training, we expect distinct clusters of cat and dog
descriptors to develop.  Visualizing the formation of such class clusters
provides an instructive view into CNN learning.
...
first compute descriptor eigendirections from a distribution of O$($2.4K$)$
validation image descriptors. Twelve-dimensional validation descriptors at
training epochs 0, 4, 8 and 16 are subsequently projected onto the dominant
three asymptotic eigendirections.  Cat and dog PCA descriptors are depicted
by red and blue points in figure 9.
