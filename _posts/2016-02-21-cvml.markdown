...
stroke colors.  The foreground colors are required to be acceptable with
...
Starting with the classic paper of Lecun et al [1], CNN approaches to digit
...
previously trained deep neural network (DNN) can work.  So we start with
the VGG-16 model [6] and replace its last layer holding 1000 ImageNet
classification categories with a new layer corresponding to the 11 labels
non-char, 0, 1, 2, ..., 9.  

Working within the caffe framework [7], we retrain the slightly modified
VGG-16 model using O(140K) synthesized image chips.  We reserve 20% of the
...
We synthesized a dozen different sets of O(140K) digit image chips and
finetuned the modified VGG-16 model on each of them.  Figure 6 displays the
training loss function and validation accuracy over 3 epochs for one
representative run of model finetuning:
...
After finetuning multiple VGG-16 models, we performed inference using the
...
finetuned on genuine SVHN and MNIST image chips.  Therefore, we finetuned
VGG-16 on a set of O(60K) SVHN digits which contained no non-character
chips.  Its recognition performance on the set of 3000 held-out SVHN chips
was 96.63%.  But this same model only correctly classified 59.44% of our
2500 test MNIST digits.  Similarly, ...[we need to compute analogous
figures for a VGG-16 model finetuned on O(60K) real MNIST digits].

These comparisons demonstrate an important potential for synthetic data to
yield a **_universal_** text classifier.  Training a classifier on particular
...
populations.  But training a classifier on synthetic data offers the hope
for recognizing text across all populations at reasonably high precision
and recall.
...
In order to improve digit recognition via synthetic character generation,
there are several avenues worth exploring in the future.  Firstly, the
confusion matrix presented in figure 9 provides some clues.  Each row in
the matrix corresponds to some SVHN digit type or non-character, while its
columns show how those types were classified by the 8-model ensemble.  The
final column in figure 9 clearly indicates that much more work is needed in
order to minimize the number of non-character image chips which are
incorrectly classified as digit chips.  Presumably, the false positive
error rate can be brought down by including more non-character chips in the
training sets.  
...
rotation were first detected and removed, we suspect that our trained
ensemble would much less frequently misclassify "7" as "1".  So to make
further progress, we believe it more sense to investigate detection and
extraction of entire numerals from images in the wild rather than continue
concentrating upon individual digit recognition.  
...
networks architected were found to yield superior performance for text
recognition applications.  Since arbitrarily large volumes of synthetic
text can be generated, training such specialized networks from scratch
should be straightforward.

We intend to return to all these interesting avenues for further research
in the future.
