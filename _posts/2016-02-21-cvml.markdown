...
Using the caffe framework [caffe], we then
retrain this modified VGG model using O(140K) synthesized image chips.  20%
of the synthetic data is reserved for model validation.  Consequently, each
of our 11 labeled categories has O(10K) synthetically generated training
samples.

The training loss function and validation accuracy are displayed over 3
epochs in figures 6a and 6b:
...
Talk about VGG performance on SVHN and MNIST when finetuning is performed
on actual SVHN and MNIST chips.  5% better than synthetic results.  But
then note performance of these specialized networks on each other.  Very
bad results.  So synthetic approach offers opportunity for developing a
