...
The DCGAN architecture we adopted closely mimics that of Radford et al [4].
The particular "super layer" combinations of convolutions, normalizations
and nonlinearities in our DCGAN discriminator and generator networks for
synthesizing 64x64 images are listed in Table 1.  The receptive fields of
these networks equals 61, and they contain O$($36M$)$ trainable parameters.
We employed similar discriminator and generator architectures for WGAN
experiments.  But following [ref], we replaced batch normalization with
layer normalization and dispensed with a final sigmoid activation in the
WGAN discriminator.
