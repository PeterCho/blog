...
several of the DCGAN faces also exhibit significant distortions.
...
rates.  In contrast, BEGAN outputs are remarkably realistic-looking.  We
consequently focus primarily upon BEGAN image generation in the remainder
of this posting.
...
$\gamma = 0.5$.$)$ But we observed the same phenomenon of evolving color
correlation images generated via DCGANs and WGANs as well.
...
intervals is shown in the figure.  Our BEGAN experiments typically suffered
less from such training instabilities than their DCGAN and WGAN
...
generated image in figure 8 corresponding to the descriptor space mean
$\vec{\mu}$ is of a caucasian woman's face.  Given that the celebA training
corpus contains O$($118K$)$ female and O$($84K$)$ male faces and that it
under-represents people of color, this average face is consistent with
semantic biases in the training data.
