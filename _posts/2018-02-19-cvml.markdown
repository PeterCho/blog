...
O(17.2M) trainable parameters.  It also depends upon a set of
hyperparameters.  Some hyperparameters are particular to BEGANs such as
diversity ratio $\gamma$ which regulates the discriminator's balance
between autoencoding real images and distinguishing synthetic from genuine
images.  $($ We typically set $\gamma = 0.5$. $)$ Others like those listed
in Table 3 enter into DCGAN, WGAN and BEGAN training experiments.  We
settled upon these hyperparameter values via trial and error.
