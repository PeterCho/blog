...
counterparts inasmuch as it assumes an autoencoder form.  The autoencoder is
comprised of an encoder which maps an input image to an intermediate bottleneck
descriptor followed by a decoder that reconstructs the original image.  The
basic architectures for the encoder and decoder are outlined in Table 2.
Following Berhelot et al [9], we incorporate skip connections in the decoder
network that concatenate the input descriptor to each upsampling layer.
Moreover, we adopt these authors' suggestion to initialize the network using
vanishing residuals [9].
...
We take the architecture for the BEGAN generator to simply be the same as
the decoder's.  A BEGAN which synthesizes 64x64 images then contains
...
for their intermediate bottleneck results.  Using these evolving
statistics, we then sample input descriptors for the BEGAN generator from a
...
GANs.  For example, the diversity ratio $\gamma$ that regulates the BEGAN
discriminator's balance between autoencoding real images and distinguishing
synthetic from genuine images.  $($ We typically set $\gamma = 0.5$. $)$
But others like those listed in Table 3 influence all DCGAN, WGAN and BEGAN
training experiments.  We settled upon the tabulated hyperparameter values
via trial and error.
...
experiments based upon their generally good qualitative appearance.
Looking at the face results, we observe that the DCGAN and WGAN outputs
contain higher spatial frequency content than their BEGAN counterparts.
But several of the DCGAN faces also exhibit significant distortions.
Similarly, DCGAN flowers look more like impressionistic paintings than
actual photographs.  WGAN results are qualitatively similar to those from
DCGAN.  Though the theoretical arguments for employing the Wasserstein loss
function are attractive [7], we empirically find no practical WGAN benefit
relative to DCGAN and much slower training rates.  In contrast, BEGAN
outputs are remarkably realistic-looking.  We consequently focus primarily
upon BEGAN image generation in the remainder of this posting.
...
As GAN networks train, we observe that generator outputs frequently exhibit
noticeable correlations which vary over time.  For example, skin tones in
generated faces and dominant colorings of generated flowers randomly change
for varying model checkpoints.  Figure 5 illustrates this phenomenon where
BEGAN training outputs are superposed on a plot of the model's approximate
convergence measure as a function of training iteration.  Abundances of
pink, white and purple flowers are produced at different times during
training.  For the particular case of BEGAN models, more variety in
generated output can be requested by increasing the diversity ratio
$\gamma$ typically at the cost of diminished quality.  But we observed the
same phenomenon of evolving color correlation for DCGAN and WGAN generated
images as well.  

![BEGAN]({{site.url}}/blog/images/gans_faces_flowers/BEGAN_flowers_learning_curve.jpg)
*Fig. 5.  BEGAN convergence measure generally decreases as training
proceeds.  But abrupt jumps in the measure's value indicate training
...
instability problem that commonly plagues many GANs.  During training,
discriminator and/or generator loss functions can exhibit major spikes.  An
example of generated outputs going bad during such unstable training
intervals is presented in the figure.  Our BEGAN experiments typically
suffered less from such training instabilities than their DCGAN and WGAN
counterparts.  But contrary to the authors' claims [9], BEGAN models do
suffer occasionally from unstable training.
...
and increased number of filter channels.  But we again found unacceptable
...
not exhibit the same degree of variety as those within the training corpus
$($ see figure 1$)$.  In particular, we never found a generated BEGAN face
with glasses [9].  But the overall quality of the faces appearing in figure
6 is noteworthy.  Unfortunately, we could not find a BEGAN architecture
which produced similarly compelling results for 128x128 flowers.
...
demonstration that generator inputs can be regarded as descriptors residing
in a linear vector space [4].  Interpolating between two different
descriptors yields a smooth morphing of the first generated image into the
second.  Since DCGANs take random noise vectors as generator inputs, the
prefered interpolation between starting and ending points in
high-dimensional descriptor spaces is along spherical arcs [17].  But we
work instead within a BEGAN descriptor space characterized by a mean vector
$\vec{\mu}$ and covariance matrix $\Sigma$.  Linearly interpolated points
between any of its descriptors lie inside the convex ellipsoidal space.
Representative examples of interpolated flowers are presented in figure 7.
...
space yields smooth transformations between generated flower images.*

It is interesting to explore the structure of BEGAN descriptor spaces which
our iteratively-derived gaussian statistics enable.  For example, the
generated image corresponding to the descriptor-space mean $\vec{\mu}$
shown in figure 8 is a caucasian woman's face.  Given that the celebA
training corpus contains O$($118K$)$ female and O$($84K$)$ male faces and
that it under-represents people of color, the average face in the figure is
consistent with biases in the training data.
...
Starting from center point $\vec{\mu}$, we can move along eigenvectors
extracted from covariance matrix $\Sigma$ for the BEGAN descriptor space.
As the representative examples of generated images in figure 9 illustrate,
such movements lead to descriptor space regions whose corresponding faces
exhibit different genders, ethnicities and facial expressions.  We can
thereby systematically map the semantic facial content which the trained
BEGAN model is capable of generating.
...
descriptor space's center yield smooth transitions between semantically
...
space statistics, we can readily perform similar manipulations on entire
eigenspace subregions.  For example, we first identify sets of
eigendirections corresponding to semantically distinct subcategories of
faces $($e.g. women with red lips and rosy cheeks, men with mustaches and
people with wide grins$)$.  Then starting from random descriptors, we
remove their initial projections along the semantically-meaningful
eigendirections.  Next, we translate the flattened descriptors along the
desired linear combinations of eigenvectors by several multiples of their
corresponding square root eigenvalues.  As figure 10 illustrates, the
engineered images exhibit female red lips, manly mustaches or toothy grins.
...
We can generate images with more specified characteristics via descriptor
...
lips with wide grins, we produce grinning female faces with red lips.
Similarly, male faces with mustaches and grins are obtained by feeding
descriptors from a particular descriptor space subregion into the BEGAN
generator.  See figure 11.
...
As a final example of synthetic image manipulation, we demonstrate
controlled modulation of faces from neutral expressions into wide grins in
figure 12.  Starting from neutral expression descriptors, we simply move
along a linear combination of eigendirections which semantically correspond
to grinning.  The sequence of faces in the figure exhibit other relatively
minor changes besides grin development.  Perhaps it is possible to find a
more precise eigendirection in BEGAN descriptor space which only affects
grin content within generated images.  We leave such refined descriptor
space investigation for future work.

![BEGAN]({{site.url}}/blog/images/gans_faces_flowers/neutral_to_grin.png)
*Fig. 12.  Movement between points in descriptor space along a particular
