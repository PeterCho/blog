...
autonomous vehicle driving, IL offers a practical means for beginning to
learn policies whose reward functions may be difficult to even formulate
[Abbeel & Ng].  Keeping in mind such potentially important real-world
applications, we are motivated to study Imitation Learning within the much
simpler context of Atari Breakout
...
conveys the ball's instantaneous two-dimensional velocity since it moves in
straight lines between occasional bounces.
...
between the ball's predicted and the paddle's current positions in the last
row then guides the teacher's stochastic policy.  The AI responds to large
positive $($negative$)$ values for $\Delta$ by moving its paddle to the
right $($left$)$, while it executes no operation if the magnitude of
$\Delta$ is close to zero.  As the movie in figure 2 illustrates, this
simple strategy enables the teacher AI to sustain long ralleys, break
...
With a decent teacher demonstrator in hand, we next investigate how well a
"student" agent can learn to mimic its behavior.  We employ deep supervised
...
different vector quantization schemes, we settled upon resetting all
non-zero coefficients to unit values.
...
empirically found that employing piecewise-linear approximations to tahn
functions work well for node activations.  Such hybrid nonlinearities run
at the same speed as ReLU or leaky ReLU thresholding, yet they lead to
better game play performance.  The neural network's 6109 biases in the
initialized to zero, and its 754,048 weights are Xavier initialized.
During training, 50% dropout is performed on hidden layer nodes to prevent
overfitting [ref].  L2-regularization is also employed to discourage
weight values from diverging.

*Fig. 4. Schematic diagram of student policy network which maps reduced ALE
breakout frames onto output paddle actions.*

Different supervised learning approaches could be implemented in order for
the student to mimic its teacher.  Since student imitation will never be
perfect, it will encounter input states not observed by its expert mentor.
So we avoided training the student network to classify input states and
output actions corresponding to just the teacher's policy.  Instead, the
student must follow its own policy and receive teacher feedback at each
timestep as to how it should have responded.



We experimented with many different reward functions before finding one
which led to effective student learning from its teacher.
...
reaches a score of 21. We compute and store state, action and reward tuples
for 20K ALE frames. Gradient descent is then performed via RMS propagation
[9], and network weights are updated. The batch of training samples is
subsequently discarded, and the next set of 20K tuples is collected.
