...
different quantization schemes, we settled upon resetting all non-zero
vector coefficients to unit values.
...
architecture is schematically illustrated in figure 4.  After experimenting
with various node activation functions, we found to our surprise that
piecewise-linear approximations to tanh functions work best.  These hybrid
activations yield better game play performance than ReLU or leaky ReLU
thresholding, and they run at the same speed.  The neural network's 6109
biases are initialized to zero, and its 754,048 weights are Xavier
initialized.  During training, 50% dropout is performed on the hidden
layers to prevent network overfitting [ref].  L2-regularization is also
employed to discourage weights from diverging.
