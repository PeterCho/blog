...
$($RL$)$ based upon policy gradient methods yielded highly effective agent
behavior.  So it is natural to hypothesize the same should be true for
Breakout.  But deep RL training via policy gradients instead leads the
agent to become trapped in poor local maxima of state space.  One of the
worst strategies learned by the agent is exhibited in the movie of figure
1:
...
neural network in our own C++ code rather than utilize an existing deep
