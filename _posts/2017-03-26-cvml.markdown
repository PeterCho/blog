...
literature.  For highly complex problems such as robot arm grappling or
autonomous vehicle driving where humans could generate expert training
data, IL offers a practical means for beginning to learn policies whose
reward functions may be difficult to even formulate [Abbeel & Ng].  Keeping
in mind such potentially important real-world applications, we are
...
straightforward to predict the ball's future last-row location ${\bf
b}(t_{\rm last \, row})$ even if it bounces off a wall on the way down.
The difference $\Delta(t) = {\bf b}(t_{\rm last \, row}) - {\bf p}(t)$
between the ball's predicted and the paddle's current pixel positions in
the last row then guides the teacher's stochastic policy.  The AI responds
to large positive $($negative$)$ values for $\Delta$ by moving its paddle
to the right $($left$)$, while it executes no operation if the magnitude of
$\Delta$ is close to zero.  As the movie in figure 2 illustrates, this
simple strategy enables the teacher AI to sustain long rallies, break
through the wall of colored bricks and rack up a large score.
...
different vector quantization schemes, we choose to reset all non-zero
coefficients to unit values.
...
empirically found that piecewise-linear approximations to tahn functions
work well for node activations.  Such hybrid nonlinearities run at the same
speed as ReLU or leaky ReLU thresholding, yet they lead to better game
play.  The neural network's 6109 biases are initialized to zero, and its
754,048 weights are Xavier initialized.  During training, 50% dropout is
performed on hidden layer nodes to prevent overfitting [ref].
L2-regularization is also employed to discourage weight values from
diverging.

*Fig. 4. Schematic diagram of student policy network which maps reduced ALE
breakout frames onto output paddle actions.*

Different supervised learning approaches may be implemented to train the
student to mimic its teacher.  Since student imitation will never be
perfect, it will encounter input states not observed by its expert mentor.
So it is prudent to avoid training the student network to classify input
states and output actions coming from just the teacher's policy.  Instead,
the student must follow its own policy and receive teacher feedback at each
timestep as to how well it responded.

We experimented with several different reward functions in order to find
feedback schemes that encourage student imitation.  For each ALE frame
input, the student agent earns a $+1$ reward if its policy action matches
that of the teacher AI.  Otherwise, it receives a penalty whose magnitude
grows as the difference $\Delta(t) = {\bf b}(t_{\rm last \, row}) - {\bf
p}(t)$ between the predicted future law-row ball and the paddle's current
pixel positions increases.  We took the penalty growth to be approximately
linear for $0 <= |\Delta| <= 12$ and then capped the penalty for $|\Delta|
> 12$.

We compute and store state, action and reward tuples for 20K ALE
frames. Gradient descent is then performed via RMS propagation [9], and
network weights are updated. The batch of training samples is subsequently
discarded, and the next set of 20K tuples is collected.
