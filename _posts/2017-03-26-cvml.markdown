...
In this blog posting, we return to working with Atari games in the Arcade
Learning Environment $($ALE$)$ [1] which provide interesting and fun
laboratories for studying deep Reinforcement Learning.  Here we focus upon
the game "Breakout" which superficially seems like a minor variant of
"Pong".  Both are single-player games which involve hitting a moving ball
with a paddle.  Points are earned in Breakout each time the ball strikes a
colored brick within 6 rows appearing near the top of the game window.  The
Breakout player's goal is to strike as many colored bricks as possible with
the ball in order to maximize the total game score.
...
leads the agent to become trapped in poor local maxima within state space.
One of the worst strategies learned by the agent is exhibited in the movie
of figure 1:
...
The movie in figure 1 vividly demonstrates the general problem exploration
vs exploitation within Reinforcement Learning.  An agent generally needs
encouragement to search unfamiliar regions of state space and avoid
entrapment in locally maximal but globally suboptimal performance zones.
Other authors have previously noted this wall-pinning malady for the
Breakout game [3].  Total scores reported in prior policy gradient [4] and
evolution strategy [5] studies of multiple Atari games also exhibit
striking dips for the particular case of Breakout.  Low scores for Breakout
among otherwise impressive Atari game results are a clear sign of local
maxima trapping.
...
with rewarding non-zero paddle movement with the hope of preventing paddle
pinning against a game wall.  But none of these simple adjustments to the
reward function had much positive impact upon the agent's learned behavior.
...
after the ball whenever it approaches the bottom of the game window is
...
we shall see in the next section, this quasi-supervised approach can
...
such long range, important applications, we are motivated to study
Imitation Learning within the much simpler context of Atari Breakout
...
encodes the correct instantaneous two-dimensional velocity as the ball
...
wall of colored bricks and rack up a large score.
...
reduce the size of the student's policy network, raw ALE frames are reduced
to sparse vectors following the image cropping, downsampling and temporal
differencing steps pictured in figure 3.  After trying a few different
vector quantization schemes, we further simplify the state vector by
resetting all its non-zero values to unity.
...
timestep as to how well it performed.
...
\le |\Delta| \le 12$ and flat for $|\Delta| > 12$.  Since the student
receives supervised feedback at each time step, we set the RL discount
factor $\gamma = 0$.
...
[9], we also calculated replay probabilities for each stored sample.
...
Similarly, states where the ball moves upwards in the game window require
less agent attention than those for where the ball moves downwards.  Replay
probabilities are heuristically assigned to encode stored states' relative
importance.
...
subsequently performed via RMS propagation [10] on the $O($8K$)$ samples
typically selected from the 20K batch members.  Once the student policy
network's weights and biases are updated, the current batch of training
samples is discarded.  A new set of 20K tuples is subsequently collected.
...
high-frequency jitter while the expert's do not.  The jittering prevents
the matching fraction from approaching unity and $|\Delta|$ from
approaching zero.
...
temporal event that is not discounted backwards in time by $\gamma$.
...
The agent's qualitative game playing abilities and limitations are
apparently within the movie of figure 8.  It generally anticipates the
gross location of the ball in the game window's last row.  The agent also
...
quantitatively and qualitatively much superior to that of the first agent
which got trapped in a bad region of state space, it still has much room
