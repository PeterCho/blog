...
Since sign of the RL loss function can be either positive or negative, we
plot the logarithm of its magnitude as a function of training epoch in
figure 5.  While the loss at individual time steps is noisy, the temporally
smoothed blue curves appearing in the figure reassuringly exhibit
decreasing trends.
...
randomly initialized, we expect an agent early on in training to take many
poor actions which are penalized by the environment.  When playing pong,
our RL agent almost always begins by losing each match to the built-in AI
...
*Fig. 9.  Movie excerpt of a "hyperactive" RL agent playing against the
