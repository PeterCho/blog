...
extracting imagery features via initial convolutional layers seems like
overkill.  After experimenting with several different network
configurations, we settled upon the one illustrated in figure XX which has
two fully connected hidden layers.  Following Kaparthy, we neglect biases
in this network.  But unlike ref[kaparthy], we include 3 softmax nodes in
the output layer that compute agent probabilities for paddle up, down and
no movement actions.
