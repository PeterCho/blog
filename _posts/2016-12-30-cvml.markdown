...
We adopt the general approach outlined by Andrej Kaparthy in his excellent
...
*Fig. XX.  Neural network which maps compressed Pong screen states into
action probabilities.*

The 35K+ weights inside the network of figure XX are Xavier initialized
according to the gaussian distribution ${\cal N}(0,\sqrt{2}/\sqrt{n_{in}})$
[He 2015].  In order to avoid weight death during training, we employ
leaky ReLU rather than conventional ReLU nonlinearities in each hidden
layer node [Maas 2013].  We also impose L2-regularization to prevent weight
values from growing without bound.

The RL agent plays against a built-in computer AI during each episode of
Pong.  The agent receives +1 reward whenever it beats the AI and a -1
penalty whenever it loses.  The episode terminates when either player
achieves a score of 21.  State, action and reward tuples are computed and
stored for 20K ALE frames.  Gradient descent of network weights is then
performed using the batch of 20K tuples.  We utilize RMS propagation to
update network weights
[Hinton Coursera].  

Rather than employing one of the currently popular deep learning software
packages, we wrote our own C++ code to implement the entire pipeline from
scratch.
...
*Fig. XX.  Movie excerpt of RL agent around training epoch 700 playing
