...
time step into a 1024 binary vector (see figure XX).

![PongRL]({{site.url}}/blog/images/pong_policy_learning/preprocessing.png)
*Fig. XX. Raw ALE frames are reduced to 32 x 32 binary state vectors
...
layers presented in figure XX.  Following Kaparthy, we neglect biases in
this network.  But unlike ref[kaparthy], we include three softmax nodes in
the network's output layer which compute agent probabilities for paddle up,
down and no movement actions.

![PongRL]({{site.url}}/blog/images/pong_policy_learning/network.png)
*Fig. XX.  Schematic diagram of the network which maps compressed Pong
screen states into action probabilities.*

The 35K+ weights inside the network of figure XX are initialized according
...
For pedagogical purposes, we chose to implement the system's neural network
...
## RL system training and playing Pong
...
<a
href="http://www.youtube.com/watch?feature=player_embedded&v=5C4_ztYXZZY"
target="_blank"><img src="http://img.youtube.com/vi/5C4_ztYXZZY/0.jpg"
alt="IMAGE ALT TEXT HERE" width="700" height="466" border="10" /></a>
*Fig. XX.  Movie excerpt of the RL agent playing against the built-in Pong
AI.  Click on image above to start the video.*
