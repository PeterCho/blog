...
time step into a 1024 binary vector as figure XX illustrates.
...
The preprocessed screen vector becomes the input layer to a policy neural
network.  As our input signal is sparse, the standard approach of
extracting imagery features via initial set of convolutional layers seems
like overkill.  So we experimented with various fully connected network
configurations and settled upon the one with two hidden layers illustrated
in figure XX.  Following Kaparthy, we neglect biases in this network.  But
unlike ref[kaparthy], we include 3 softmax nodes in the output layer that
compute agent probabilities for paddle up, down and no movement actions.

![PongRL]({{site.url}}/blog/images/pong_policy_learning/network.png)
*Fig. XX.  Neural network used to map Pong screen states into action
probabilities.*

The 35K+ weights inside the neural network of figure XX are Xavier initialized
according to the gaussian distribution ${\cal N}(0,\sqrt{2}/\sqrt{n_{in}})$
[He 2015].  In order to avoid weight death during training, we utilize
leaky ReLU rather than conventional ReLU nonlinear thresholding in each
hidden layer node [Maas 2013].
