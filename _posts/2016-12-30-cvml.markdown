...
networks [refs].  After this breakthrough, computer games in general and
Atari games in particular have become popular benchmarks among RL
practioners for developing new algorithms and evaluating their performance.
In this post, we explore the ability of deep RL methods based upon policy
gradients to play the basic Atari game Pong.

Our investigation follows the general approach outlined by Andrej Kaparthy
in his "Pong from Pixels" blog writeup [ref].  Since Pong screens are so
...
differenced pixels values into zeros and ones.  As figure illustrates, our
Pong environment state is therefore represented by a 1024 binary vector at
each time step.
...
*.  [Deep mind Atari paper refs go here]
