---
layout: post
title:  "Teaching an RL Agent to Lazily Win at Atari Pong"
...
of SL is to improve the agent's prediction function which maps from input
...
In order to begin converting this SL loss expression to an RL loss
function, we first rename variables following figure 1 and substitute state
$s_t$ for input $x_i$.  The agent is assumed to follow a stochastic policy
$\pi(a|s)$ which specifies the probability it will execute action $a$ when
the environment is in state $s$.  We therefore replace SL output label
$y_i$ by RL sampled action $a_t$ and probability distribution $p$ by $\pi$.
...
in this equation reduces long term dependencies by downweighting future
rewards.  The second state-action value function
...
The reinforcement learning loss function incorporates an advantage function
estimator

$$ \hat{A}_t = r_t + \gamma r_{t+1} + \gamma^2 r_{t+2} + \cdots - V^\pi_t $$

as training sample modulators:

$$ L_{RL} = - {1 \over T} \sum_{t = 0}^{T-1} \hat{A}_t \log \pi(a_t |
s_t). $$

Probabilities for good actions with positive advantages are consequently
increased within this RL loss expression, while bad actions with negative
...
should decrease as training proceeds.
...
The "policy gradient" approach to reinforcement learning may be applied to
a wide range of problems.  Provided stateful world, reward delay,
exploration vs exploitation and step size problems are not too severe, it
yields interesting results.

## Deep RL system 

Games provide good test environments for reinforcement learning.  After
...
upon Deep Q networks [refs].  Following this breakthrough, Atari games in
...
performance.  Here we explore the policy gradient method's ability to learn
to play the Atari game Pong.

We adopt the general approach outlined by Andrej Kaparthy in his lucid
"Pong from Pixels" blog post [ref].  Since Pong screens are so simple, we
...
the agent's paddle to prevent its disappearing in differenced images if the
...
seems like overkill.  So we instead experimented with various fully
connected network configurations and settled upon the one with two hidden
layers presented in figure 3.  Following ref[Kaparthy], we neglect biases
in this network.  But [kaparthy], we include three softmax nodes
in the network's output layer which compute agent probabilities for paddle
up, down and no movement actions.
...
The 35K+ weights inside the network of figure 3 are initialized according
to the gaussian distribution ${\cal N}(0,\sqrt{2 / n_{in}})$ [He 2015].  In
order to avoid weight death during training, we employ leaky ReLU rather
than conventional ReLU nonlinearities in each hidden layer node [Maas
2013].  We also impose L2-regularization to discourage weight values from
...
and a -1 penalty if it misses.  The episode terminates when either player
reaches a score of 21.  State, action and reward tuples are computed and
stored for 20K ALE frames.  Gradient descent is then performed via RMS
propagation, and network weights are updated [Hinton coursera].  The batch
of training samples is subsequently discarded, and the next set of 20K
tuples is collected.

This RL system for Pong is fairly simple.  It depends upon a set of
hyperparameters whose empirically determined values are listed in Table 1.
For pedagogical purposes, we choose to implement the system's neural network
from scratch in C++ rather than employ one of the many existing deep
learning software packages.  Fortunately, the network is sufficiently
...
## Jittery versus lazy RL agent Pong playing

When training neural networks from scratch and searching for reasonable
hyperparameter values, it is important to have multiple diagonstics to
...
nonlinearities.  After changing from ReLU to leaky ReLU activation
functions, we no longer saw weight movement cease.  Instead, random weights
typically evolved like those displayed in figure 4 for one particular
training session of our Pong RL agent.
...
Since the RL loss function can be either positive or negative, we plot the
logarithm of its magnitude as a function of training epoch in figure 5.
While the loss at individual time steps is noisy, the temporally smoothed
blue curves appearing in the figure reassuringly demonstrate decreasing
trends.

![PongRL]({{site.url}}/blog/images/pong_policy_learning/montage_loss.jpg)
*Fig. 5. Plots of "jittery" [LHS] and "lazy" [RHS] RL agent loss function
...
$-\gamma^n$ where $n$ denotes the number of ALE frames until it misses the
...
over all episode time steps should increase.  The temporally smoothed
curves in figure 6 illustrate such learning behavior.

![PongRL]({{site.url}}/blog/images/pong_policy_learning/montage_erewards.jpg)
*Fig. 6. Plots of "jittery" [LHS] and "lazy" [RHS] RL agent average
discounted eventual rewards per episode plotted as functions of training
epoch.*

As the RL agent becomes more skilled, its volleys against the AI grow
longer.  Pong game duration therefore goes up during early network
training.  It is worth noting that the agent can win games against the
built-in AI even if its averaged discounted eventual reward is slightly
negative.  In particular, long volleys which the agent ultimately loses
decrease its eventual reward score more than short winning volleys increase
its averaged tally.  Of course, the agent's overall game score also
increases as its pong playing ability improves.  More interestingly, the
number of ALE frames per game actually goes down when the agent's expertise
allows it to rapidly win.  Figures 7 and 8 exhibit all of of these learning
behaviors.

![PongRL]({{site.url}}/blog/images/pong_policy_learning/montage_rewards.jpg)
*Fig. 7. Differences between game scores for "jittery" and "lazy" RL
...
*Fig. 8. Number of ALE frames per Pong episode for "jittery" and "lazy" RL
agents plotted as functions of training epoch.*

<a href="http://www.youtube.com/watch?feature=player_embedded&v=xDoBMdJOYco"
target="_blank"><img src="http://img.youtube.com/vi/xDoBMdJOYco/0.jpg"
alt="IMAGE ALT TEXT HERE" width="700" height="466" border="10" /></a>
*Fig. 9.  Movie excerpt of a "jittery" RL agent playing against the
built-in Pong AI at end of episode 3600 [epoch = 854.87].  Click on image
...
alt="IMAGE ALT TEXT HERE" width="700" height="466" border="10" /></a>
*Fig. 10.  Movie excerpt of a "lazy" RL agent playing against the built-in
Pong AI during at end of episode 5000 [epoch 920.43].  Click on image above
to start the video.*
...
alt="IMAGE ALT TEXT HERE" width="700" height="466" border="10" /></a>
*Fig. 11.  Movie excerpt of a "super lazy" RL agent playing against the
built-in Pong AI.  Click on image above to start the video.*
...
Proc. 30th ICML ((2013)).](https://web.stanford.edu/~awni/papers/relu_hybrid_icml2013_final.pdf)
