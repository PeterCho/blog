...
into training sample modulation coefficients:
...
Eqn 4 represents the "policy gradient" approach to reinforcement learning
which may be applied to a wide range of problems.  Provided stateful world,
reward delay, exploration vs exploitation and step size problems are not
too severe, it yields interesting results.
...
benchmarks among RL practioners for developing new algorithms and
...
agent's paddle to prevent its disappearing in differenced images when the
...
Since the RL loss function sign can be either positive or negative, we plot
the logarithm of its magnitude as a function of training epoch in figure 5.
The loss at individual time steps is obviously noisy.  But the temporally
...
in traiing which are penalized by the environment.  When playing pong, our
RL agent almost always begins by losing each match to the built-in AI
...
improves.  Eventually, the number of ALE frames per game actually goes down
once the agent's expertise enables it to rapidly win.  All of these
learning behaviors are exhibited in Figures 7 and 8.
...
is actual agent play against the built-in AI.  The movie excerpt presented
in figure XX shows the endgame for episode 3600 which corresponds to epoch
854.87 in the LHS plots of figures 5 through 8.  Around this point in
training, the Pong agent's game playing performance is just starting to
grow superior to its opponent.  Indeed, the agent beats the AI 21 to 20 at
the end of the movie in figure XX.  With more training, the RL agent
discovers a systematic defect in the AI's response.  It learns to exploit
this weakness and consistently crushes its opponent after 2000 training
epochs.
...
built-in Pong AI at end of episode 3600.  Click on image above to start the
YouTube video.*

Our deep agent learns to win at Atari Pong via policy gradients.  But as
the movie excerpt clearly illustrates, its paddle movement exhibits
significant high frequency jitter.  The agent's jerky motions seem
unnecessary, and we label its playing "hyperactive".  It's natural to ask
if some sort of low-pass temporal filtering can be performed to reduce the
jitter.
...
where $y[n]$ denotes the paddle's coordinate at the $n$th timestep.  After
some empirical tests, we set proportionality coefficient $\alpha = 1E-3$
for the paddle acceleration penalty term.
...
induces the RL agent to play pong more like the way a human would.  In
particular, the agent frequently selects the "no operation" action when the
pong ball is located far away from its paddle.  As the movie in figure XX
illustrates, the agent's new policy expends much less paddle energy than
its hyperactive counterpart.  We call the more relaxed RL agent which
avoids unnecessary paddle accelerations a "lazy" Pong player.
...
We close by noting that one RL agents trained with the anti-acceleration
reward signal stumbled upon the ultimate lazy player's solution.  In
...
win Pong games 21 to 0 without ever moving!  We note that the AI opponent
did not always oblige the RL agent by shooting balls straight at its fixed
location.  So this "super lazy" learned policy is not as good as the one
corresponding to the reward curve on the RHS of figure 7.  But as the movie
in figure XX illustrates, this randomly discovered strategy for playing
Pong is very amusing.
...
Berkeley course CS 294, $\small($ 2015 $\small)$](http://rll.berkeley.edu/deeprlcourse-fa15/docs/2015.08.26.Lecture01Intro.pdf)

2.  [J. Schulman, N. Heess, T. Weber and P. Abbeel, "Gradient Estimation
using Stochastic Computation Graphs", arXiv: 1506.05254v3 
$\small($ 2016 $\small)$.](https://arxiv.org/pdf/1506.05254v3.pdf)
...
Learning" $\small($ 2013 $\small)$
.](https://www.cs.toronto.edu/~vmnih/docs/dqn.pdf)

5.  [V. Mnih et al, "Human-Level Control through Deep Reinforcement
Learning", Nature 518 $\small($ 2015 $\small)$
529.](http://files.davidqiu.com/research/nature14236.pdf)

6.  [A. Kaparthy, "Deep Reinforcement Learning: Pong from Pixels",
$\small($ 2016 $\small)$ .]( http://karpathy.github.io/2016/05/31/rl/)

7. [K. He, X. Zhang, S. Ren and J. Sun, "Delving Deep into Rectifiers:
Surpassing Human-Level Performance on ImageNet Classification",
arXiv:1502.01852v1 $\small($ 2015
$\small)$.](https://arxiv.org/pdf/1502.01852.pdf)

8. [A.L. Maas, A. Y. Hannun, A.Y. Ng, "Rectifier Nonlinearities Improve
Neural Network Acoustic Models", Proc. 30th ICML $\small($ 2013
$\small)$.](https://web.stanford.edu/~awni/papers/relu_hybrid_icml2013_final.pdf)

9. [G. Hinton, "Neural Networks for Machine Learning", Slide 29 from
Lecture 6 Coursera notes, $\small($ 2012
$\small)$.](https://www.coursera.org/learn/neural-networks)
