...
their manually assigned labels.
...
colorings.  Each input noise vector to a CGAN consequently yields a
meaningful "family" of conditional output views.
...
pictures.  To measure diversity within image sets, we follow ref. [16] and
...
images on a scale between 0 [radically different] to 1 [completely
identical].  For each flower class, we compute the median MS-SSIM score for
500 random pairs of images as a measure of intra-class diversity.

Table 2a lists median MS-SSIM values for our five flower classes among
genuine images as well as synthetic images before and after binary
...
progenitors.  Table 2b lists averaged flower classifier accuracies for the
same three categories of images.  Not surprisingly, its scores indicate
CGAN images filtered by the secondary binary classifier are more
photo-realistic than their pre-filter progenitors.  The generative and
discriminative figures of merit in these two tables provide quantitative
measures analogous to recall and precision.
...
*Table 2a. Median MS-SSIM scores per flower class calculated from genuine and synthetic
imagery.*
...
*Table 2b. Flower classifier accuracies per flower class calculated from genuine and
synthetic imagery.*


## Flower classification based on genuine plus generated imagery

With a CGAN flower generator in hand, we now have a new source of training
imagery that requires no human effort to label.  We can use the CGAN to
synthesize arbitrary numbers of daisy, dandelion, rose, sunflower and tulip
images which may be combined with the original 4K genuine pictures to train
new flower classifiers.  Table 2b indicates that the discriminative values
of synthetic images filtered by the secondary binary classifier are
comparable to those of genuine flower images.  However table 2a
demonstrates that the diversity of the generated CGAN flowers is
significantly less than that of the original genuine training imagery
corpus.  So we may reasonably expect CGAN data augmentation to initially
improve flower classification performance.  But as numbers of artifical
flowers overwhelms numbers of genuine flowers within the training corpus,
we anticipate CGAN data augmentation will hurt flower classification.

In order to chart out the data augmentation landscape, we form five
distinct training corpera containing the original 4K genuine flower photos
along with 0K, 4K, 8K, 12K and 16K synthetic images.  The CNN described in
the first section of this blog posting is then trained with these different
inputs.  Inference is performed on the withheld set of 500+ genuine test
images using each of the trained CNN classifiers.  Figure 8 displays
individual and averaged flower classification accuracies as a function of
...
The classifier's performance noticeably improves from 87.7% to 91.7% when
the total number of training images is doubled via augmentation.  Modulo
statistical fluctuations, it modestly continues to increase up to 92.9%
when the training corpus is quadrupled in size.  But further augmentation
with CGAN imagery leads to a decline in flower classification accuracy.  So
with our particular CNN operating on daisy, dandelion, rose, sunflower and
tulip pictures, we find CGAN data augmentation improves classification
performance by more than 5%.

In closing, we note that "real-world" training sets are often imbalanced
among different classes.  Practical considerations can render some
classifier data inputs more difficult to gather than others.  It would be
interesting to study how well Conditional GAN data augmentation could help
compensate for a priori unbalanced training sets.  We leave these questions
to future work.
...
16.  [A. Odena, C. Olah and J. Shlens, "Conditional Image Synthesis with
