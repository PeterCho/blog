...
Following the highly successful VGG-16 model [12], we set the pixel
...
and 256 such filters.  The fully-connected fifth and sixth layers both
contain 256 nodes.  The seventh layer contains just 3 nodes corresponding
to our non, male and female face categories.  Filtering in layers 1-6 of
the network is followed by a nonlinear ReLU operation.  Max pooling is
performed after the first three convolutions which reduces image chip pixel
width and height each by half.  In the final ouput layer, softmax
probabilities are computed for our 3 classes.  The one with the highest
score yields the classification label output corresponding to the image
chip input.
...
step [13].  Dropout probabilities grow in magnitude with increasing layer
depth as follows: 0.1, 0.25, 0.25, 0.5, 0.5, 0.5, 0.5.  We also employ
L2-regularization to minimize overfitting.

We implement this CNN using the caffe framework [10].  Its weights and
...
size = 100.  We set the initial learning rate = 0.01 and decrease it by
0.33 after every 30K iterations.  We also experimented with adjusting the
weight decay parameter and settled upon lambda = 0.001.
...
It is interesting to note that a number of our test images contain faces
...
images [1].  Figures 8a and 8b demonstrate that a CNN can also successfully
classify genders for a number of such 2D faces.

![FaceGender]({{site.url}}/blog/images/face_gender/results/female_2D_correct.png.png)
*Fig. 8a.  Examples of "2D" test image chips originating from paintings,
line drawings and photos were the CNN correctly classified as female.*

![FaceGender]({{site.url}}/blog/images/face_gender/results/male_2D_correct.png.png)
*Fig. 8b.  Examples of "2D" test image chips originating from paintings,
line drawings and photos were the CNN correctly classified as male.*

Of course, if input image quality is poor, our system cannot confidently
return any classification result.  For instance, the machine marks facial
genders for all of the chips in figure 9 as unknown:

![FaceGender]({{site.url}}/blog/images/face_gender/results/padded_unsure_classifications.png)
*Fig. 9. Examples of test image chips for which the CNN was significantly
uncertain about gender.  The top [bottom] row contains female [male] image
chips.*

And our CNN also definitely makes classification mistakes.  Representative
examples of incorrectly classified female and male image chips are
presented in figures 10a and 10b.  For several of the image chips in these
figures, it is difficult for even a human to determine gender without
seeing the context of the full images from which they were excised.  It is
often hard to tell gender for very young and old human subjects as well as
for people whose faces are obscured by masks or markings.  Small pixel
sizes and croppings of head shots also render sex determination difficult.
But in other failure cases, most humans would have no trouble determining
...
successfully localized and classified are presented in figure 11.  They
...
Figures 12a and 12b present more challenging examples which include crowd
