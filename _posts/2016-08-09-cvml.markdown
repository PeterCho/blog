---
layout: post
title:  "Classifying Gender in Internet Images via a Convolutional Neural Network for Faces"
...
In our previous blog post, we detected faces within internet photos using
instance segmentation [1].  In this entry, we continue to accumulate and
investigate a sizable corpus of "faces in the wild".  But here our primary
focus is upon classifying rather than detecting faces.

Humans around the world may be described in terms of characteristics which
change on time scales ranging from seconds (e.g. mood) to decades
(e.g. age).  But for most people, gender labels are constant throughout
their lifetimes.  Moreover, physical differences exist between male and
female faces.  So gender arguably represents one of the most basic human
attributes.  We seek to build a classifer which can identify gender from
facial imagery input.

We begin by extending the dataset of internet images from our earlier face
detection studies.  To date, we have collected and annotated 13740
photos which intentionally exhibit a high degree of variety.  2815 of the
...
The labeled faces belong to people with differing attributes (e.g. gender,
age, ethnicity) performing activities (e.g. socializing, swimming, singing)
within disparate settings (e.g. hospital rooms, memorial services, sport
stadiums).  Moreover, people's visages are oriented in variable poses
(e.g. facing camera, looking upwards, profile view), and many are partially
occluded by objects (e.g. eyeglasses, drinking cups, other people's heads).
We also intentionally increased the number of internet images containing
faces from paintings and drawings relative to our previous face
investigation.

While IMGLAB is convenient for drawing bounding boxes around objects, it
does not currently support hierarchical assignment of multiple labels per
box.  So we developed our own gender attribution tool based upon the 3D
OpenSceneGraph toolkit [4].  Our program imports and displays face bounding
boxes generated via IMGLAB. A user may press the number pad "1", "2" or "0"
keys to initialize gender attributes for all bounding boxes in an image to
male, female or unknown.  After a particular box is selected via the mouse,
its gender attribute may be altered to male, female or unknown by pressing
the "1", "2" or "~" keys near the upper left of a computer keyboard.  We
used this attribution tool to assign gender labels to 40K+ facial bounding
boxes.
...
Yellow bounding boxes mark persons for whom we were significantly uncertain
about their sex.  As we attempted to accurately attribute thousands of
different people appearing in internet images, we were surprised by how
often it is difficult to determine gender from facial and other appearance
cues.  In particular, we found distinguishing between baby boys and girls
to be nearly impossible if their clothing provided no clear cultural hints.
In other cases, one can deduce that blurry patches in images must
correspond to human faces.  But such patches may contain too little
information for a gender label to be confidently assigned.  All such faces
marked with "unknown" genders are ignored by our classification pipeline.

![FaceGender]({{site.url}}/blog/images/face_gender/training/image_03815_crowd_genders_01.png)
*Fig. 1. An internet image containing faces attributed with female, male and unknown gender labels.*

The bounding boxes in figure 1 are snuggly wrapped around individual faces.
But information valuable for sex determination such as hair style, neckware
and upper-body clothing often resides outside the figure's boxes.  We
consequently double all facial bounding box widths and heights to capture
more pixels useful for gender classification.

The resulting human head regions come in a wide variety of sizes.  But a
convolutional neural network requires uniformly-sized imagery input.  In
particular, the CNN that we develop below imports image chips of pixel size
96x96.  Therefore, inputs larger than 96x96 are subsampled down.  On the
other hand, inputs smaller than 96x96 are superposed onto black
backgrounds.  (To improve CNN performance, we double the pixel sizes of all
chips smaller than 48x48 before superposing them onto black backgrounds.)
Examples of head chips extracted from figure 1 and spatially resized to
96x96 are presented in figure 2.
...
In order to expand the volumes of our training and testing data, we
searched online for other sets of gender-labeled face chips.  While many
facial databases can be found on the internet [5], relatively few come with
gender metadata.  One relatively recent dataset may be obtained from the
OUI-Adience Face Image Project from which we extracted 17457 headshots
[6,7].  The sources for these data are flickr albums.  Adience images are
...
included 369 head shots of women available from the Iranian Face Database
[8,9].  After combining all these head shots, we have a total of 59837
image chips:  31166 are labeled as female, and 28671 are labeled as male.

We randomly reserve 2.5% of the 59K+ chips for validation and another 7.5%
...
chips appear in the leftmost column of this tableau.  Their output variants
are randomly translated, rotated, flipped and color-modulated relative to
the original images.*
...
Random boxes are also heavily biased towards enclosing image regions with
sizable entropies.  We thus generate 99204 non-face image chips, and we
...
![FaceGender]({{site.url}}/blog/images/face_gender/training/padded_facenet_architecture.png)
...
pooling with kernel size 2 and stride 2 is performed after the first three
convolutions.  Each pooling step reduces image chip pixel width and height
by half.  In the final layer, softmax probabilities are computed for the 3
classes.  The one with the highest score yields the classification label
output corresponding to the image chip input.

The network pictured in figure 5 contains a total of 10,575,136 weights and
1283 biases whose values we train from scratch.  In order to combat
overfitting this large number of variables, we intentionally perform
dropout after each ReLU step [12].  Dropout probabilities grow in magnitude
with increasing layer depth as follows: 0.1, 0.25, 0.25, 0.5, 0.5, 0.5,
0.5.  We also employ L2-regularization to minimize overfitting.

We implement this CNN using the caffe framework [13].  Its weights and
biases are initialized with random values drawn from a Gaussian
...
less than the cyan curve's.  So dropout plus L2-regularization clearly
...
chips.  We found that the CNN had greater difficulty classifying females
than males.  This is surprising given that our corpus of unaugmented image
chips contains 2K+ more females than males.  Perhaps the frequent presence
of facial hair makes male faces intrinsicially easier to identify than
female faces.  We also discovered that softmax probabilities for correctly
labeled validation chips were generally higher than those for their
incorrectly classified counterparts.

It is unreasonable to expect any automated system to perform perfectly all
the time.  But classifiers should return confidence values along with their
...
label that lies below these thresholds, the result is deemed too uncertain
to be trusted.
...
segmentation could successfully detect such "flat" faces in internet
...
billboards correctly classified as female.*
...
The confusion matrix in Table 1 quantifies our classification system's
performance.  The CNN has little difficulty separating non-face from
human-face inputs.  It consequently could be used to improve precision for
detectors which find faces within internet imagery.  The 86% correct
classification rate for male faces is also respectable.  It compares
favorably with recent CNN gender results reported for smaller and less
challenging data sets than ours [6].  On the other hand, our CNN struggles
to correctly identify female faces, and it gives up on over 12% of female
test inputs.  By lowering the female softmax threshold below 0.58, we could
increase the correctly-classified female fraction to around 80%.  But such
a threshold relaxation would come at the unacceptable cost of also
increasing the incorrectly-classified female fraction.
...
*Table 1. Classification results for a held-out truth set of 1037 non-face,
1812 male face and 1687 female face image chips.*

After aggregating subtotals within Table 1, we find 84.46% of our test
images were correctly classifed and 9.72% were incorrectly classified.  The
CNN was uncertain about and assigned no label to the remaining 5.82% of the
test set.

## Detecting and classifying faces in internet imagery

The face detector developed in our previous blog posting can
be combined with the face gender classifier described here.  We may then
pass a brand new set of "images in the wild" not previously seen by either
the detector or classifier through the end-to-end system.  Instance
...
Several faces which were both successfully detected and classified are
...
detected and assigned correct gender labels.  Red [blue] bounding boxes
...
the camera direction.  We observe a few faces for which the classifier was
uncertain about a person's sex.  But generally for the pictures in figure
12, the detector has more trouble finding faces than does the classifier
determining their genders.

![FaceGender]({{site.url}}/blog/images/face_gender/results/good_most_faces.png)
*Fig. 12a. Example test images for which no detected face was assigned an
incorrect gender label.  Red [blue] bounding boxes enclose female [male]
faces.  The CNN was uncertain about gender for faces marked by green
bounding boxes.  A few faces in these images were not detected.*

![FaceGender]({{site.url}}/blog/images/face_gender/results/OK_faces.png)
*Fig. 12b. Example test images for which no detected face was assigned an
incorrect gender label.  Several faces in these images were not detected.*

Finally, the examples in figure 13 illustrate failures of both the detector
and classifier.  They include cases where faces were not detected as well
as incorrect gender labels for detected faces.  We leave improving both
facial detection and classification for future work.

![FaceGender]({{site.url}}/blog/images/face_gender/results/bad_faces.png)
*Fig. 13. Example test images in which some detected faces were assigned
