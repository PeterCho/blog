...
In our previous blog post [1], we localized faces within internet photos
using instance segmentation.  In this entry, we continue to accumulate and
...
identify gender based primarily upon facial imagery input.
...
images which intentionally exhibit as much variety as possible.  2815 of
the images originated from the WIDER collection [2], while the remaining
...
attributes (e.g. gender, age, ethnicity) performing varying activities
(e.g. socializing, swimming, singing) in disparate settings (e.g. hospital
rooms, memorial services, sport stadiums).  Moreover, people's visages are
oriented in variable poses (e.g. facing camera, looking upwards, profile
view), and many are partially occluded by objects (e.g. eyeglasses,
drinking cups, other people's heads).  We also intentionally increased the
number of internet images containing faces from paintings and drawings from
our previous face investigation.
...
the 3D graphics toolkit OpenSceneGraph [5].  It imports and displays face
...
The sources for these data are flickr albums, and Adience images are
...
54760 image chips with male or female labels.
...
Following the highly successful VGG-16 CNN [12], we set the pixel
...
192, 224 and 256 such filters.  Max pooling is performed after the first
three convolutions which reduces the image chip's pixel width and height
each by half.  The fully-connected fifth and sixth layers both contain 256
nodes.  Filtering within layers 1-6 of the network is followed by a
...
We implement this network using the caffe framework [10].  The weights and
...
train the network using stochastic gradient descent with momentum = 0.9 and
minibatch size = 100.  We set the initial learning rate = 0.01 and decrease
it by 0.33 after every 30K iterations.  We also experimented with adjusting
the weight decay parameter to minimize overfitting and settled upon lambda
= 0.001.
...
the cyan curve.  So dropout plus L2-regularization clearly prevent our CNN
from overfitting.
...
We settled upon the CNN architecture and hyperparameters presented in the
previous section after performing multiple runs on our validation set of
1617 image chips.  To our surprise, we found that the CNN had greater
problems with incorrectly classifying females than males.  We also
discovered that the softmax probabilities for correctly labeled validation
chips were generally higher than those for their incorrect counterparts.
...
threshold values of 0.52 and 0.58 upon softmax probabilities for male and
female classification labels.  If our CNN outputs a gender label and
confidence score that lie below these thresholds, the result is declared
too uncertain to be trusted.

We next pass a reserved set of 4536 image chips through the CNN to assess
its performance.  Representative examples of test chips whose facial
genders were correctly classified are presented in figures 7a and 7b.  They
...
In other cases like those appearing in figure 8, input image quality was
sufficiently poor that our system could not confidently return any
classification result.  The machine marks these chips' facial genders as
uncertain:

![FaceGender]({{site.url}}/blog/images/face_gender/results/padded_unsure_classifications.png)
*Fig. 8. Examples of test image chips for which the CNN was significantly
uncertain about gender.  The top [bottom] row contains female [male] image
chips.*

Of course, our CNN also makes mistakes.  Representative examples of
incorrectly classified female and male image chips are presented in figures
9a and 9b.  For several of the image chips in these figures, it is
difficult for even a human to determine gender without seeing the context
of the full images from which they were excised.  It is often hard to tell
...
of head shots also render sex determination difficult.  But in other
...
*Fig. 9a. Examples of female test image chips which the CNN incorrectly
classified as male.*

![FaceGender]({{site.url}}/blog/images/face_gender/results/incorrect_male_2_female_classifications.png)
*Fig. 9b. Examples of male test image chips which the CNN incorrectly
...
the precision for detectors which localize faces within images in the wild.
The 86% correct classification rate for male faces is also respectable.  It
compares favorably with recent CNN gender results reported for smaller and
less challenging data sets than ours [adience].  On the other hand, our CNN
...
0.58, we can increase the correctly-classified female fraction but at the
cost of also increasing the incorrectly-classified fraction.
...
It is instructive to combine together the facial localization detector
presented in our blog posting with the facial gender classifier developed
in this entry.  We collect a brand new set of test images not seen before
by either subsystems' neural networks.  Instance segmentation attempts to
wrap bounding boxes around the faces of genuine humans, and the classifier
attempts to determine the faces' genders.  Several faces which were both
successfully localized and classified are presented in figure 10.  They
include relatively simple examples where all human subjects face straight
towards the camera.

![FaceGender]({{site.url}}/blog/images/face_gender/results/good_all_faces.png)
*Fig. 10. Example test images in which all faces were automatically
localized and assigned correct gender labels.  Red [blue] bounding boxes
enclose female [male] faces.*

Figures 11a and 11b present more challenging examples which include crowd
scenes, head coverings obscuring face visibility and faces significantly
oriented away from the camera's direction.  In these cases, our detector
has more trouble finding faces than does our classifier in determining
their genders.  

![FaceGender]({{site.url}}/blog/images/face_gender/results/good_most_faces.png)
*Fig. 11a. Example test images for which no localized face was assigned an
...
*Fig. 11b. Example test images for which no localized face was assigned an
incorrect gender label.  Several faces in these images were not localized.*

Finally, the examples in figure 12 illustrate failures of both the detector
...
*Fig. 12. Example test images in which some localized faces were assigned
...
10.  See http://caffe.berkeleyvision.org.

11.  See ethereon.github.io/netscope/quickstart.html.
