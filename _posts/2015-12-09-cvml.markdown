...
edge with a very large weight links two images which are nearly identical.
On the other hand, two images connected by an edge with almost zero weight
are totally different.  So given two images, we need a way to compute their
similarity.
...
bundler examples [2]), we derive the image network pictured below:
...
large and interesting networks of images can be formed [3].  But if we want
