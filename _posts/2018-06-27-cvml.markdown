...
dataset [3] and other pictures we manually harvested from the internet
[4].  For inpainting experiments, we superpose boxes of various sizes and
...
scratch via the TensorFlow deep learning framework [9].  To facilitate
...
imagery reconstruction.  We do so in the next section where we consider
image-to-image translations for inpainting and colorizing problems.

## Conditional image GAN results

Our basic approach to CGAN training is to force the discriminator and
generator to first learn about gross object shapes, colors and textures.
While training proceeds, object details are refined as low-pass filtering
is relaxed.  By the end of training, the networks are encouraged to
incorporate high spatial frequency content into their inputs and outputs.
Figures XX and XX illustrate these trends for both inpainting and
colorizing CGANs.

![INPAINT_COLOR]({{site.url}}/blog/images/inpainting_colorization/training_evolution_inpainting.jpg)
*Fig. XX.  Generator inputs and outputs at equally spaced intervals
...
reconstruction on new examples which it never previously saw.  CGAN results
are then guaranteed to not represent mere network memorizations of specific
input-output pairs.  So from here on, we only work with inference images
which were separated off from their training counterparts and genuinely
test CGAN generalization capabilities.

Figure XX illustrates inpainting reconstructions based upon the L1 and CGAN
loss terms by themselves as well as their combination following the
schedule in figure XX.  The L1 reconstructions recover gross shape and
color information inside the masked box regions but completely miss high
spatial frequency details.  The pure CGAN term reconstructions look grossly
distorted.  In contrast, the combined L1 + CGAN results look remarkably
plausible.  While reconstructions do not precisely match the progenitor
flower views in the rightmost column of fig XX, we believe their filling in
of missing information is compelling.
...
flower image.  COL 2: Images reconstructed via a pure L1 loss function 
COL 3: Images reconstructed via a pure CGAN loss function. COL 4:
Images reconstructed via the loss function with both L1 and CGAN terms
whose coefficients follow the schedule in figure XX.  COL 5: Progenitor
flower images.*

Fig XX presents 8 more examples of generally successful flower image
inpainting.  The figure's first column contains partially masked input
images.  The second column presents flower reconstructions while the third
contains progenitor pictures.  These examples suggest that the inpainting
CGAN has learned nontrivial knowledge about flower morphology, texture and
colors.
...
The basic goal of image inpainting is to recover masked pixel content as
faithfully as possible.  It consequently tries to map one degraded input to
one reconstructed output.  We consequently did not incorporate any random
noise input within the inpainting CGAN's generator.  On the other hand,
image colorizing can potentially one greyscale input to many RGB outputs.
So for the colorizing CGAN generator, we do concatenate the bottleneck
layer within the U-net architecture with a random noise descriptor to
encourage synthetic image variety.  As figure XX illustrates, some
colorized outputs look qualitatively similar to the progenitor RGB
pictures.  But in other cases like those shown in figure XX, the trained
CGAN converts greyscale inputs to colored outputs which nontrivially differ
from the original RGB pictures.  In a few instances, the synthetically
colored views look more "realistic" to our eye than the actual progenitors!

The inpainting and colorizing results presented so far look quite
encouraging.  But we have also observed several cases where image
reconstruction obviously failed.  Figures XX and YY display CGAN outputs
that appear highly unrealistic.  Perhaps some of these poor inference
examples lie so far away from training data samples that the CGAN is unable
to reasonably extrapolate.  In other cases, the networks clearly struggle
with complex scenes containing many different object types.   As we have
seen in our previous GAN studies, adversarial networks can fail to produce
sensible outputs for a sizable fraction of their inference inputs.
...
After looking at O$($100$)$ inpainting and colorizing inference results, we
estimate ?% qualitatively appear reasonable.  This is consistent with the
Amazon Turk study findings reported in [1].  Clearly much more work remains
to be done to improve image-to-image translation robustness in the future.
