...
greyscale inputs which may look more realistic than the original colored
image progenitors.
...
via a luminosity transformation.  Representative examples of original,
...
*Fig. 1.  LEFT: Examples from our 256 $\times$ 256 flower imagery corpus.
CENTER: Randomly sized and colored boxes superposed at various locations
inside each picture.  RIGHT: Greyscale versions of original RGB images.*
...
Isola *et al.* found such skip connections lead to much higher quality
...
The CGAN's discriminator network has a relatively simple architecture that
converts pairs of original and conditional images into probabilities that
the progenitor picture is genuine rather than synthetic:
...
*Table 1b.  Discriminator network used to analyze $($original,
...
ii.  Pixelwise normalization within generator and discriminator superlayers
significantly improves GAN training stabilization [ref].

iii. Skip connections indicated within generator superlayers are implemented
by concatenating encoder tensors onto similarly-sized decoder tensors.
...
we choose to concatenate the bottleneck's $Z_{dim}$ tensor with a second
random tensor with the same dimensions.  In this case, we double the number
of channels following the bottleneck layer in Table 1a as needed.  

We work within the TensorFlow framework [] to train this network's 5.6M
parameters.  CGAN training depends upon several hyperparameters whose
empirically-derived  values are listed below:
...
The original and deformed flower pictures are split 90%/10% into training
and testing subsets.
