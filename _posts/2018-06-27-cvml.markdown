...
greyscale inputs which look more realistic than the colored image
progenitors.
...
never observed discriminator or generator learning curves suffer from
...
reconstructions then cannot merely represent network memorizations of
specific input-output pairs.  So from here on, we work with inference
images which were separated off from their training counterparts and
genuinely test deep learning generalization capabilities.
...
and colors inside the masked box regions but completely miss high spatial
frequency details.  The pure CGAN term reconstructions look badly
...
whose coefficients follow the schedule in figure 3.  COL 5: Progenitor
flower images.*

Figure 7 presents several more examples of generally successful flower
image inpainting.  The figure's first column contains partially masked
input images.  The second column presents L1 + CGAN reconstructions while
the third displays progenitor pictures.  These examples demonstrate that
the inpainting networks have developed a nontrivial understanding of flower
...
faithfully as possible.  It consequently tries to map a degraded picture
...
image colorizing can potentially map a greyscale picture onto many RGB
...
*Fig. 8a.  LEFT COLS: Greyscale versions of various inference flower images.
...
*Fig. 8b.  LEFT COLS: Greyscale versions of various inference flower images.
CENTER COLS: Images colorized by the trained conditional image GAN
generator.  RIGHT COLS: Progenitor images.*

The conditional image GAN results presented so far look quite encouraging.
But we have also observed several obvious image reconstruction failures.
Figures 9 and 10 display inpainting and colorizing examples that appear
highly unrealistic.  Some of these poor inference results might lie so far
away from training data samples that the CGAN was unable to reasonably
extrapolate.  In other cases, the networks clearly struggle with complex
scenes containing many different object types.  As we have seen in our
previous GAN studies, adversarial networks can unfortunately fail to
produce sensible outputs for a sizable fraction of their inference inputs.

![INPAINT_COLOR]({{site.url}}/blog/images/inpainting_colorization/bad_inpaintings_montage.png)
*Fig. 9. Poor GAN inpainting results.  Columns in this figure follow the
pattern of figure 7.*

![INPAINT_COLOR]({{site.url}}/blog/images/inpainting_colorization/bad_colorizations_montage.png)
*Fig. 10. Poor GAN colorizing results.  Columns in this figure follow the
pattern of figures 8a and 8b.*
...
Amazon Turk study findings reported in [1].  Clearly more work remains to
be done in the future to improve image-to-image translation robustness.
