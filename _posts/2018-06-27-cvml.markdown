...
flower image inpainting and colorizing.

## Encouraging conditional image GAN results

![INPAINT_COLOR]({{site.url}}/blog/images/inpainting_colorization/training_evolution_inpainting.jpg)
*Fig. XX.  Sample generator inputs and outputs at equally spaced intervals
...
reconstruction on new examples of downgraded pictures which it never
previously saw.  CGAN results are then guaranteed to not represent mere
network memorizations of specific input-output pairs.  So from here on, we
only work with testing images which were separated off from their training
counterparts.
...
color information within the masked box regions.  But their blurry forms
missed high spatial frequency details.  The pure CGAN term reconstructions
look grossly distorted.  While the combined L1 + CGAN results do not
precisely match the progenitor flower views appearing in the rightmost
column of fig XX, their filling in of missing information is remarkably
plausible.
...
Fig XX presents 8 more examples of qualitatively successful flower image
inpainting results.  In each row of the figure, the first column shows a
partially masked input image.  The second column illustrates the
reconstructed flower image while the third column contains the progenitor
picture.  These examples strongly suggest that the CGAN has learned
nontrivial knowledge about flower morphology, texture and colors.
...
## Discouraging conditional image GAN results



Unfortunately, other cases of image inpainting are much less encouraging.
Figure XX presents obviously poor reconstructions coming from the same
trained CGAN.  

CGAN cannot handle highly complex scenes containing many different beyond
flowers.  Training corpus was not sufficiently broad to handle such scenes.
