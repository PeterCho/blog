...
In November 2016, Isola, Zhu, Zhou and Efros posted a paper entitled
"Image-to-Image Translation with Conditional Adversarial Networks" which
demonstrated that conditional GANs can be used to transform an input image
into a corresponding output image [1].  These authors persuasively argued
that just as a single verbal concept can be expressed in multiple
languages, so can a single visual concept be rendered in multiple views.
Isola et al developed a common framework to translate between image pairs
and applied it to several problems in computer vision and computer
graphics.  Their results were stunning.

In this blog posting, we build upon the work of ref. [1] while focusing
upon image inpainting and colorizing applications.  In particular, we
follow the basic strategy of Isola et al but modify several of their
implementation details.  For instance, we introduce time-dependence into
CGAN training via initial imagery blurring and loss-term coefficient
modulation.  We observe such temporal evolution improves inpainting
results' quality.  Furthermore, we incorporate random noise descriptor
inputs into the CGAN generator.  We find this source of non-determinism can
lead to synthesized RGB outputs from greyscale inputs which sometimes look
more realistic than the original colored image progenitors.

We work with a corpus of O(15K) flower images which come from Oxford 102,
blah as well as others we manually harvested from the internet.  The input
imagery set is split 85%/15% into training and testing subsets.  For
inpainting experiments, we superpose randomly sized and randomly colored
boxes at various locations within each flower image.  For colorizing
experiments, we simply convert each RGB image into its greyscale
counterpart via a luminosity transformation.  Representative examples of
original, inpainting and colorizing versions of flower images are displayed
in figure 1.
...
*Fig. 1.  LEFT: Examples from 256 $\times$ 256 flower imagery corpus.
CENTER: Randomly sized and colored boxes superposed at random locations
inside each flower picture.  RIGHT: Greyscale versions of original RGB
images.*
...
*Fig. XX.  Schematic diagram for CGAN generator's Unet architecture.  This
diagram is a minor variant of figure 3 from [Isola].*
...
<table style="width:100%">
  ...
  <tbody>
    <tr>
      ...
      <td style="text-align: center">256 $\times$ 256 $\times$ $($1 or 3$)$</td>
    </tr>
    ...
    <tr>
      ...
      <td style="text-align: center">16 $\times$ 16 $\times$ $($56 + 48$)$ </td>
    </tr>
    ...
    <tr>
      <td style="text-align: left">Upscale, Conv5, Pixelwise norm, Leaky Relu</td>
      <td style="text-align: center">32 $\times$ 32 $\times$ $($48 + 40$)$ </td>
    </tr>
    ...
    <tr>
      <td style="text-align: left">Upscale, Conv5, Pixelwise norm, Leaky Relu</td>
      <td style="text-align: center">64 $\times$ 64 $\times$ $($40 + 32$)$ </td>
    </tr>
    ...
    <tr>
      <td style="text-align: left">Upscale, Conv5, Pixelwise norm, Leaky Relu</td>
      <td style="text-align: center">128 $\times$ 128 $\times$ $($32 + 24$)$ </td>
    </tr>
    ...
    <tr>
      <td style="text-align: left">Upscale, Conv5, Pixelwise norm, Leaky Relu</td>
      <td style="text-align: center">256 $\times$ 256 $\times$ $($24 + 16$)$ </td>
    </tr>
...
<table style="width:100%">
  ...
  <tbody>
    <tr>
      ...
      <td style="text-align: center">256 $\times$ 256 $\times$ $($3 [input]
+ 1 or 3 [conditional] $)$</td>
    </tr>
    <tr>
...
conditionally synthesize and analyze 256 $\times$ 256 images.*
...
*Fig. XX.  Blur schedule.*

![INPAINT_COLOR]({{site.url}}/blog/images/inpainting_colorization/loss_coeffs_schedule.jpg)
*Fig. XX.  Time evolution of L1 and GAN loss term coefficients.*

![INPAINT_COLOR]({{site.url}}/blog/images/inpainting_colorization/learning_curves.jpg)
*Fig. XX.  Generator and discriminator learning curves.*
...
whose coefficients follow the schedule in figure XX.  COL 5: Original
flower images.*
...
conditional image GAN generator.  RIGHT: Original images.*
...
generator.  RIGHT COLS: Original images.*
