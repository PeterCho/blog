...
pre-aligned CelebA photos.  It consequently represents a more challenging
...
pictures qualitatively similar to those in figure 1.
...
images.  Table 1 lists the best "super layer" combinations of linear
...
iii.  Rather than feeding descriptors into the generator drawn from random
distributions during training, we chose instead to import descriptors coming
from the discriminator's next-to-last superlayer.  During inference, the
generator may be supplied with noise inputs originating from random uniform or
...
vi.  After trying batch normalization [12], layer normalization [13] and
...
both the discriminator and generator networks.  These authors slowly
...
We explore an alternative approach to gradual GAN training in which image pixel
size and network architectures are kept fixed.  Instead, we blur discriminator
input images with a gaussian kernel whose radius is a function of training
epoch.  As the GAN hyperparameters listed in Table 2 indicate, we take the
kernel's initial radius to equal 10% of training image pixel size.  During the
first 200 epochs, the blur kernel linearly ramps down to zero.  No blurring of
input imagery is performed for the last 300 training epochs.  This low-pass
filtering schedule is plotted in figure 3.
