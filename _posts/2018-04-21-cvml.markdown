...
approach to increasing GAN image resolution.  Unlike Karras *et al.*, we
keep the discriminator and generator network architectures fixed throughout
training.  But the sharpness of images we feed into the discriminator
starts out low and gradually increases as training proceeds.  So the GAN
...
training coupled with bad generated image rejection yields synthetic
pictures qualitatively similar to the genuine photos appearing in figure 1.
...
catastrophic mode collapse.  We note that no normalization is applied
within the first discriminator superlayer.
...
starts with 4 $\times$ 4 input pictures and a single convolutional
superlayer in both the discriminator and generator networks.  The authors
slowly increase image size and numbers of network superlayers.  Karras *et
al.* persuasively argue that incrementally training a GAN allows it to
first discover large-scale structure in an image distribution and later
learn fine details.  Such progressive training yields better performance
than requiring a GAN to learn content from all scales at once.
...
ramps down to zero.  No blurring of input imagery is performed during the
last 300 epochs.  This low-pass filtering schedule is plotted in figure 3.
...
equally spaced epoch intervals are displayed in the figure's top and bottom
rows.  The GAN needs time to learn how to respond to input imagery with
different resolutions.  There is consequently a delay of approximately 125
epochs between input and output pictures with comparable bluriness.  But
after training for 500 epochs, the GAN exports bird images which are
grossly comparable to their import counterparts.

![BIRDGAN]({{site.url}}/blog/images/bird_dcgan/discrim_in_gen_out.png)
*Fig. 4.  Discriminator inputs $($top row$)$ and generator outputs
$($bottom row$)$ at training epochs 0, 125, 250, 375 and 500.*
...
one-third of the 192 $\times$ 192 pictures as reasonable generation
products.  Bird beaks, wings and tails look realistic, and plumage coloring
is detailed among the good instances.  Moreover, the successful synthetics
exhibit nontrivial variety in their foreground and background contents.  In
figure 5, most birds appear to be sitting on tree branches or walking on
the ground.  But we have observed other cases where birds are standing near
or floating on bodies of water.  So the GAN has learned to produce water
foul in addition to tree dwellers and ground walkers.
...
similar strengths and weaknesses to ours.  We further note that Kerras *et
al.* trained their bird GAN with 9 times more images than we used.  Among
the bird pictures copied from ref. [1] and reproduced here in figure 6,
roughly one-third look reasonable.  But the remaining two-thirds suffer
from significant distortions.  So *consistently* generating photo-realistic
bird pictures with O$($200 $\times$ 200$)$ pixels remains an outstanding
challenge.
...
labeled data were next used to train a CNN whose architecture resembles
that of the first deep network described in ref. [14].  We subsequently
...
flying birds pass the classifier stage.  Perhaps a GAN could be taught to
generate real-looking birds in flight if the number of such training
...
distorted.  But as the last row in figure 9 demonstrates, one bird's beak
sometimes plausibly transforms into another bird's tail.  Such morphings of
distinct bird body parts into each other are amusing to observe.
