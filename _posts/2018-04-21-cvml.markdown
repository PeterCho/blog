...
We conducted an empirical search for GAN designs and training procedures
which yield good synthetic bird pictures.  Our initial attempts started
with the DCGAN architecture of Radford *et al.* that outputs 64 $\times$ 64
images [8].  Over the course of O$($100$)$ experiments, our neural
networks' forms and training schedules evolved towards more stable
configurations which could accommodate images with larger pixel sizes.
...
found for a GAN that synthesizes 192 $\times$ 192 images.
...
iv.  After trying out Relu, Leaky Relu [9], Exponential Linear Unit [10]
...
v.  After trying out batch normalization [11], layer normalization [12] and
...
In ref. [1], Karras *et al.* describe a training methodology for GANs which
starts with 4 $\times$ 4 input pictures and progressively increases image
size while adding layers to the networks.  These authors persuasively argue
that such incremental training enables a GAN to first discover large-scale
structure within an image distribution.  The GAN then allowed to slowly
learn finer scale detail rather than having to handle all-scale content at
once.

We explore an alternative approach to such gradual GAN training in which
image pixel size and network architectures are kept fixed.  Instead, we
blur input images with a gaussian kernel whose radius is a function of
training epoch.  As the GAN training hyperparameters listed in Table 2
...
ramps down to zero.  No blurring of input imagery is subsequently performed
during the last 300 training epochs.  Figure 3 plots this low-pass
filtering schedule.
...
*Fig. 3.  $($Top row$)$: Discriminator's blurring kernel radius linearly
...
9.  [A.L. Maas, A.Y. Hannun and A.Y. Ng, "Rectifier Nonlinearities Improve
