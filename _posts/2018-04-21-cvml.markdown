...
We conducted a search over GAN designs and training techniques which create
bird pictures.  Our initial attempts started with the DCGAN architecture of
Radford *et al.* that outputs 64 $\times$ 64 images [8].  Over the course
of O$($100$)$ experiments performed within the Tensorflow deep learning
framework [9], our neural networks' forms and training procedures evolved
towards more stable configurations which could accommodate larger images.
Table 1 lists the best "super layer" combinations of linear projections,
convolutions, transposed convolutions, normalizations and nonlinearities we
found for a GAN that produces 192 $\times$ 192 images.
...
coming from the discriminator's next-to-last superlayer.  During inference,
the generator may be supplied noise inputs originating from random uniform
...
v.  After trying out Relu, Leaky Relu [10], Exponential Linear Unit [11]
and Tanh functions for activations in all non-final superlayers, we found
Leaky Relu nonlinearities led to the best GAN performance.

vi.  After trying out batch normalization [12], layer normalization [13] and
...
300 training epochs.  This low-pass filtering schedule is plotted in figure
3.
...
We take some consolation in observing that bird results presented in
ref. [1] exhibit qualitatively similar strengths and weaknesses to ours.
We further note that Kerras *et al.* trained their bird GAN with 9 times
more images than we used.  Among the bird pictures copied from ref. [1] and
reproduced here in figure 6, roughly one-third look reasonable.  But the
remaining two-thirds range suffer from significant distortions.  So
consistently generating photo-realistic bird pictures with O$($200 $\times$
200$)$ pixels remains an outstanding challenge.
...
which the GAN itself provides.  But we saw no obvious correlation between
output generator images and their corresponding discriminator scores.
We next tried passing generator results through a trained autoencoder with
...
The former category contained 1308 members, while the latter had 2692.
This labeled data was next used to train a CNN whose architecture resembles
...
instructive to compare the variety of final system outputs in figure 8 with
their system input counterparts' in figure 1.  For instance, we note that
no pictures of flying birds survive the classifier stage.  Perhaps a GAN
could be taught to generate quasi-realistic flying bird pictures if the
number of such training examples were significantly increased.  But the
overall distributions of bird shapes, colorings and textures in the two
figures are qualitatively similar.  We leave quantitatively comparing GAN
input and output image distributions for future work.
...
Examples of such induced bird morphings are provided in figure 9.  It is
interesting to note that rendered birds at intermediate points in the
transitions can exhibit qualitatively new attributes such as head colorings
and tail feathers not seen within the beginning or ending pictures.
Moreover, intermediate views between left-facing and right-facing birds
often look distorted.  But as the last row in figure 9 illustrates,
sometimes a bird's beak can plausibly morph into another bird's tail.  Such
distinct bird body part morphings are amusing to see.
