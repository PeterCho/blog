...
advertisements, street signs, trucks and vans, etc.  Text in these pictures
...
segmentation then offers a means for separating different lines of text and
...
into the trained DNNs.  As figure 9b demonstrates, the classifiers then
correctly localize a larger range of character sizes when they operate on
an image pyramid formed from a single picture.
...
max pooling, average pooling or more sophisticated weighted pooling [8].
...
classified as text lying inside the truthed bounding boxes to the total
number of pixels classified as text.  If we were to adopt an analogous
measure for recall based solely upon pixel counts, performance assessment
would be biased towards text strings with large pixel sizes.  So we instead
compute the fractional coverage of bounding boxes by pixels classified as
text within every test image.  We thus form precision and recall
distributions over the set of 307 test images.
...
Since localization usally represents the first step in most end-to-end text
recognition systems, it is practically more important for recall to be as
close to unity as possible even if precision suffers.  False positive text
detections could be elimiminated by more specialized classifiers of
letters, digits, words and/or phrases at subsequent stages in an end-to-end
pipeline.  In contrast, it would be much more difficult to later find words
if they are completely missed at the initial nomination stage.  While 72.9%
precision and 86.5% recall are still far from perfection, we note that
these values are significantly larger than the 54% precision and 42% recall
[9] achieved by the researchers who developed the data set in [7].
...
classifier trained on synthetic English phrases.  The Chinese text images
