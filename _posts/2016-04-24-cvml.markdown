---
layout: post
title:  "Detecting Text-in-the-Wild via Synthetic Phrase Generation and
...
setups, text-in-the-wild presents many variables with which an AI system
must contend.  These include wide varieties of text fonts, colors,
...
are *a priori* unknown.  Single characters, multiple abutted business names
...
arbitrary input images which represents a prerequisite for text
recognition.  Given recent successes in supervised image classification,
one obvious attempt to solve this first stage of processing would be to
amass a training collection containing thousands of manually labeled
examples.  This brute force approach is currently popular among big
companies with huge budgets and sizable human resources.  But we are more
interested in exploring alternative sources of training data which do not
require massive human effort to produce.

Text falls within a special category of objects seen in the physical world
which are primarily computer-generated.  Nearly all text appearing in
...
localize text-in-the-wild.  Our approach is similar in spirit to others
which synthesize text for classifier training purposes [1].  But we will
incorporate more variability than earlier efforts did and output entire
phrases rather than just individual words.

We start by developing a corpus of English phrases which will become inputs
to our synthetic image engine.  Since names frequently appear in the
physical world, we initially download lists of male and female first names
along with common surnames.  We then randomly combine these parts to form
...
by initials.  In addition to synthesized people names, we also include into
our text corpus company names found via internet searches.

Numerals frequently appear in the physical world.  So we randomly generate
integers containing 1-6 digits as well as decimal numerals containing 1 or
2 digits after a decimal point.  For 20% of the decimal numbers, we prepend
a "$" in order to simulate price tags often posted in store fronts and at
gas stations.  Telephone numbers containing 10 or 7 digits are also
randomly generated.  Some of the 3-digit area codes are separated by
parentheses, while some of the 7-digit telephone numbers are separated by
hyphens.

Text-in-the-wild often contains many words spanning multiple lines.  So we
harvest relatively long phrases from news reports, computer blogs, science
articles and creative writing stories posted online over the past two
decades.  Excess white space is removed from these input sources, but
punctuation marks are intentionally retained.  Phrases containing 1 to 15
words are extracted from these text files.
...
uppercase Latin letters over character widths ranging from 5 - 100 pixels
is rejected.  Fonts corresponding to cursive handwriting are also
intentionally included.  We ended up with 700+ fonts to use for
synthesizing text.

Our synthetic phrase generator is based upon ImageMagick's text-to-image
capabilities [2].  The generator first randomly selects some particular
...
non-white character to emulate some widely-spaced store signs and
advertisements seen in the wild.  Others have carriage returns inserted
after each letter in order to generate vertically aligned text which also
...
reasonable estimates for all the rendered characters' widths.  A temporary
image is virtually created to hold the entire input string within a single
textline.  Individual character widths are found by subtracting lengths of
virtually rendered substrings containing c+1 and c characters within the
wide, virtual image.  (Index c ranges from 0 to one less than the number of
characters in the input text phrase.)  Once rendered character widths are
known, the beginnings of each text line within the actual synthetic image
are determined via horizontal and vertical profiling.  Pixel bounding boxes
around each synthetic character are then readily derived.
...
properties may be generated.  For instance, pixel bounding boxes
...
own mask values.  Figure 2a illustrates such character-within-word
relationships for the text phrase in figure 1.  The resulting mask
...
into a virtual camera, we use ImageMagick to generate and export their 2D
imageplane renderings.

Following this entire procedure, we generate 131K+ foreground text phrases
plus accompanying image masks.  Figure 3 displays several examples of these
...
We consequently download 1000+ internet pictures to serve as backgrounds
for our synthesized text image foregrounds.  The pixel size for each
background photo is required to be larger than 1Kx1K.  Many of the
background views are of real-world outdoor or indoor scenes.  Others are
...
Given a text-free internet image, foreground text chips are quasi-randomly
superposed onto the backdrop texture.  At each candidate text location,
entropy density is calculated within the background image.  The candidate
text position is rejected if the local background entropy density is too
high.  Similarly, average foreground and background RGB values are
calculated at nominated text sites.  If their color contents do not
sufficiently differ, the candidate text location is rejected.  The bounding
box for any new foreground chip is also not allowed to overlap any previous
text bounding boxes.  The number of foreground overlays within each
composite varies over the interval [9,27].

Once a composite image is formed, each of its pixel's RGB values is
fluctuated by a nonzero amount of gaussian noise.  A sizable fraction of
the composites are also randomly blurred.  To our surprise, we observed
that such composite degradation significantly improves later semantic
segmentation classifier performance on test images containing genuine text
content.
...
Corresponding 8-bit mask indicates relative horizontal character locations
within words via greyscale colorings.*
...
We thus produce over a quarter-million tile triples.  As we shall see in
the next section, these image plus mask tiles can be used to train deep
semantic segmentation networks to localize text-in-the-wild in arbitrary
input images.

## Semantic segmentation of text-in-the-wild

We utilize the DeepLab framework [3,4] built on top of Caffe [5] to
...
randomly-shuffled associations between image tiles and masks.  8K of the
...
which is a variant of the VGG-16 model [6].  We adopt a batch size of 30,
retain DeepLab's default 0.9 momentum and 0.0005 weight decay parameters,
employ a Stochastic Gradient Descent optimizer, and utilize a step learning
rate policy.  We found that training and validation results were relatively
insensitive to modifications of the learning rate.  So we initialize the
learning rate to 0.0045 [0.005] when fine-tuning the DNN for horizontal
word-character [vertical text-line] segmentation.  Figure 6 plots the loss
functions for these two DNN models over 3 training epochs.
...
text-in-the-wild, we test them against a challenging set of ground-truthed
images [7].  It consists of 307 color photos of outdoor urban scenes with
significant text content appearing on store fronts, billboard
advertisements, street signs, truck sides, etc.  Text in these pictures
exhibits a wide variety of fonts, colors, sizes, distance ranges, angular
orientations, physical locations and illumination conditions.
...
in cases like those in figure 8 where multiple text lines are closely
packed together, horizontal character-in-word pixel classifications may
bleed from one line into another.  Vertical segmentation then offers a
means for separating different lines of text and the words contained within
them.
...
for 2 test images.  Colored tinting for horizontal characters appearing on
...
within the right hand-sides of the image pairs.*

It is important to note that DeepLab's modified version of the VGG-16
network has a receptive field size of 128 pixels.  Moreover, our
synthesized characters have O(100) pixel maximum widths.  So our fine-tuned
DNNs struggle when segmenting letters or digits whose scales exceed 100
pixels.  Similarly, their performance deteriorates on strings with very
small pixel sizes.  The mask results on the right side of figure 9a exhibit
both of these maladies.  As a simple remedy, we feed half-sized and
double-sized versions of each picture along with the original-sized image
into the trained DNNs.  The classifiers then correctly localize a larger
range of character sizes when they operate on an image pyramid formed from
a single picture (see figure 9b).
...
image with 1024x1360 pixel size.  The "S" in "Subway" is approximately 100
pixels wide.  In contrast, small billboard characters near the top of the
image are roughly 6 pixels wide.*
...
image's pixel size.  They could be aggregated into a single mask via max
pooling, average pooling or more sophisticated weighted pooling [8].  But
for purposes of comparing our text localization results with ground truth,
we simply convert all character classes into binary text versus non-text
pixel labels.  After taking the union of text labels across the 3 pyramid
levels, we form a single binary mask that represents the consolidated
output from our detection system.  We quantitatively compare these masks
with ground truth in the next section.
  
## Text segmentation results vs ground truth

The 307 test images available for download at the URL listed in [7] are
accompanied by axis-aligned bounding boxes which were manually generated by
human annotators (see figure 10).  These bounding box metadata can be used
to evaluate the precision and recall of our text localization system. We
compute a precision value for each test image from the ratio of pixels
classified as text lying inside the bounding boxes to the total number of
pixels classified as text.  If we were to adopt an analogous measure for
recall based solely upon pixel counts, performance assessment would be
biased towards text strings with large pixel sizes.  So we instead compute
the fractional coverage of bounding boxes by pixels classified as text
within every test image.  We thus form precision and recall distributions
over the set of 307 test images.

![TextLocalization]({{ site.url }}/blog/images/text_segmentation/total_segs/montage_text_img0305___bboxes_0305.jpg)
*Fig. 10.  Text segmentation results for a test image aggregated
from original, half-sized and double-sized pixel resolutions.  Ground truth
bounding boxes are colored yellow on the right hand-side of the image pair.*
...
as figures 10, 11a and 11b illustrate.  In these cases, some of our
system's valid detections are erroneously scored as false positives.  In
other instances, axis-aligned bounding boxes do not snuggly surround text
strings in the test images as can be seen in figure 11b.  Consequently,
some pixels marked with default non-text labels are incorrectly scored as
false negatives.  Of course in other cases, the truth data do reveal
genuine false positive and false negative mistakes made by the trained DNN
classifiers such as those listed in figure 12.


![TextLocalization]({{ site.url}}/blog/images/text_segmentation/total_segs/montage_text_img0294___bboxes_0294.jpg)
*Fig. 11a.  Text segmentation results in this test image actually
...
*Fig. 11b.  Text segmentation results for in this test image more
...
building facade ornamentations incorrectly classified as text.*
...
distributions exhibit long tails.  We consequently quote their median
rather than average values in order to condense them to single numbers:
Precision = 72.9%, Recall = 86.5%.

![TextLocalization]({{ site.url }}/blog/images/text_segmentation/recall_prec/montage_precision_dens___recall_dens.jpg)
*Fig. 13.  Text pixel precision and bounding box recall distributions for
the 307 test images from [7].*

Since localization usually represents the first step in end-to-end text
recognition systems, it is practically more important for recall to be as
close to unity as possible even if precision suffers.  False positive
detections could be elimiminated by more specialized classifiers of
letters, digits, words and/or phrases at subsequent stages in a complete
processing pipeline.  On the other hand, it would be much more difficult to
later find words if they are completely missed at the initial nomination
stage.  While 72.9% precision and 86.5% recall are far from perfect, we
note that these values are significantly larger than the 54% precision and
42% recall [9] achieved by the researchers who developed the data set in
[7].

## Segmentation of non-English text

We close this blog entry with an intriguing observation regarding
localization of text-in-the-wild written in languages other than English.
Nearly all of the non-numerical synthesized phrases used to train our DNNs
were in English words.  And none of them contained characters from outside
the Latin alphabet.  Yet gross spatial relationships among characters
within and between words transcend the English language.

It is consequently interesting to see the classifiers' results in figure 14
for text written in Chinese, Greek and Arabic.  I cannot read any of these
languages.  But the color-coded segmentations in this figure look
...
a classifier trained on synthetic English phrases.  The Chinese text images
are part of a truth set assembled by the authors of [10].  The Greek and
Arabic images come from [11].*
...
4.  G. Papandreou, L.-C. Chen, K. Murphy and A.L. Yuille,
*Weakly- and Semi-Supervised Learning of a DCNN for Semantic Image 
Segmentation*, arXiv:1502.02734 (2015).

5.  See http://caffe.berkeleyvision.org.

6.  K. Simonyan and A. Zisserman, *Very Deep Convolutional Networks for
Large-Scale Image Recognition*, arXiv 1409.1556, (2015).

7.  See http://research.microsoft.com/enus/um/people/eyalofek/text_detection_database.zip.

8.  L.-C. Chen, Y. Yang, J. Wang, W. Xu and A.L. Yuille, *Attention to
Scale: Scale-aware Semantic Image Segmentation*, arXiv:1511.03339 (2015).

9.  B. Epshtein, E. Ofek and Y. Wexler, *Detecting Text in Natural Scenes
with Stroke Width Transform", CVPR (2010), 2963.

10.  C. Yao, X. Bai, W. Liu, Y. Ma and Z. Tu, *Detecting Texts of Arbitrary
Orientations in Natural Images*, CVPR (2012), 1083.

11.  Video frames extracted from October 2013 PBS NewsHour broadcasts.  See
