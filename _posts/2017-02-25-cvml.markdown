...
letters within our word corpus.  According to figure 2, the ten most common
letters listed in descending order are "e", "i", "a", "n", "s", "o", "r",
"t", "l" and "c".  One strategy for playing hangman is to guess letters
following the single-letter distribution.  This simple approach
consequently provides a baseline for comparison against learned game
playing behaviors.
...
use "one-hot" encoding to convert letters "a" - "z" into a 26-dimensional
basis.  All letters are then evenly separated in this sparse vector space.
But we prefer to instead employ a more compact representation which
captures English language structure.
...
$($corresponding to ascii value 32$)$ filling their empty slots.

As the game proceeds, we also need to keep track of incorrectly guessed
letters.  We consequently append 16-dimensional descriptors for each wrong
letter onto the word vector representation.  As our game rules permit a
maximum of 9 wrong letters, the state vector's total length is 432 =
...
nodes corresponding to letters 'a' through 'z'.  The final action sampling
node resets the probabilities of any already-chosen letters to zero, and it
renormalizes the remaining unchosen letters' probabilities so that they sum
to one.  Selected letter output may then either be randomly sampled from
the renormalized distributions or taken to be the most likely choice.
...
and fractions of games won.  As figure 4 illustrates, expected discounted
reward peaks around training epoch 0.7 and then declines with further
training:
...
policy network training in order to maximize RL agent game playing
performance.
...
$\pi(a)$.  The validation results in figure 5 are based upon stochastically
sampling $\pi$ to yield the RL agent's next guessed letter.  From the
figure, we see an optimal validation win fraction around 24% occuring near
training epoch 0.3.  But we empirically found that the game win fraction
for the validation data set increases to 40% around training epoch 0.49 if
we instead always select the most probable letter at each turn.  So we
adopt this quasi-deterministic letter selection rule for the trained RL
agent.
...
instructive to first examine the agent's decision making as a function of
...
guesses according to the sequence "e", "o", "n", "s", "r", "g", "a", "l",
"t" and "i".  This ordering of the top ten letters differs significantly
from that derived from the relative frequency of individual letters
appearing in figure 2.  After the first ten turns, the agent's letter
selection distributions start exhibiting multi-modal behavior.

The RL agent's pattern of letter choices can be seen in the YouTube video
in figure 7 as it plays five representative episodes.  In the movie, we see
the unknown word initially represented by a set of underscore characters.
As the game proceeds, correctly guessed letters colored green appear inside
the word.  Green letters are also added to the list of previously sleected
