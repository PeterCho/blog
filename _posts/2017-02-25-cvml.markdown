...
ideas and techniques using games as instructive and fun laboratories.  Here
we focus upon the classic word game "hangman" which involves guessing the
letter content of *a priori* unknown words.  Unlike the maze and Atari Pong
environments in our previous RL investigations, the hangman world state is
only partially observable.  So this word game introduces some interesting
differences from the image-based games of our past two postings.

We start by downloading a text file containing 355K English words from the
github site listed in ref. [1].  We choose to ignore any words with fewer
than 5 or more than 18 letters.  Words containing non-letter characters are
also discarded.  The resulting trimmed corpus is then split into training,
...
relatively simple.  But others like those in Table 1 are uncommon, and some
words originate from foreign languages that have presumably been absorbed
into English:
...
work with the language model of Jozefowicz et al [2] which was trained on
the One Billion Word Benchmark [3].  In particular, we use these authors'
python code [4] to generate 16-dimensional descriptors for each character
corresponding to ascii values 0 through 255 [5].
...
in figure 3.  The softmax action layer contains 26 nodes corresponding to
...
[6].  50% dropout on the hidden layers is also performed during training
to combat network overfitting [7].  L2-regularization is further employed
...
during training: loss function, fraction of word letters correctly guessed,
expected future discounted reward and fraction of games won.  In the end,
we found training the policy network from scratch for a maximum of 11 turns
yielded the best results as measured by expected discounted reward and
fractions of games won.  As figure 4 illustrates, expected discounted
reward peaks around training epoch 0.7 and then decline with further
training:
...
The game winning fraction values displayed in figure 5 rise and fall even
more rapidly.  The figure's training and validation curves also exhibit no
signs of network overfitting.  So we need to perform early stopping on
policy network training in order to maximize its game playing performance.
...
In addition to experimenting with training the neural network and
monitoring its progress, we also investigated different options for
exploiting the learned policy.  The validation results pictured in figure 5
are based upon stochastically sampling $\pi(a)$ to yield the RL agent's
next guessed letter.  From the figure, we see an optimal validation win
fraction around 24% occuring near training epoch 0.3.  But we empirically
found that the game win fraction for the validation data set significantly
improves to 40% around training epoch 0.49 if we instead always select the
most probable letter at each turn.  So we adopt this quasi-deterministic
approach for having the trained RL agent play the hangman word game.

## Game play and performance

Once the RL agent had finished learning, we evaluated its performance
playing full hangman games on the held-out set of 26K testing words.  It is
instructive to first examine the trained agent's decision making as a
function of game turn.  Figure 6 presents its letter selection
distributions for game turns 1, 4, 7, 10, 13 and 16:

![Hangman]({{site.url}}/blog/images/hangman/montage_cropped_p_actions.jpg)
*Fig. 6.  RL agent letter selection distributions at different turns during a 
hangman word game.*

The distributions for the first ten turns in each hangman game are almost
delta functions.  In particular, the RL agent nearly always guesses its
first ten letters according to the sequence "e", "o", "n", "s", "r", "g",
"a", "l", "t" and "i".  This subset of letters and their ordering differ
nontrivially from that which would be derived from the relative frequency
of individual letters appearing in corpus words plotted in figure 2.

This pattern of letter choices is exhibited within the YouTube movie of the
RL agent playing 5 representative episodes in figure 7.  As the video
plays, we see the unknown word initially represented by a set of underscore
characters.  As the game proceeds, correctly guessed letters colored green
appear inside the word.  Green letters are also added to the list of
previously sleected characters along with incorrect guesses colored red.
Near the bottom of the game window, the top five probabilities for the next
letter coming from policy $\pi$ are displayed.  The letter with the highest
probability is colored purple and represents the agent's next guess.
...
*Fig. 7.  Movie of deep RL agent playing 5 episodes of the hangman word
game.  Click on image above to start the YouTube video.*

By the end of the movie, the RL agent successfully guesses the letter
contents of three out of the first five random test words.  When the
agent's performance is measured on the 26K test word data set, its average
winning game fraction equals 0.401.  Test words containing mroe letters are
generally easier for the agent to solve, for they usually lead to more
chances to guess.  The agent's performance as a function of test word
letter count is plotted within the histogram on the left side of figure 8.

![Hangman]({{site.url}}/blog/images/hangman/RL_agent_vs_monogram_win_dist.jpg)
*Fig. 8.  Fraction of hangman games won plotted as a function of number of
...
For comparison, the histogram on the right side of the figure illustrates
the fraction of games won if the agent simply follows the single letter
...
is two times better than that of the latter.  So the RL agent has learned
something nontrivial about how to play the hangman word game.

Nevertheless, there is certainly much room for reinforcement learning
improvement.  In particular, we would like to see more intelligent
dependence of policy letter selection upon input state.  Recent work by
Bachman et al outlines a more sophisticated approach to solving guessing
games than the relatively simple one we have presented here [9].  Among the
applications considered by these authors is the hangman word game.
Unfortunately, it is difficult to quantitatively compare their system's
performance with ours given their qualitatively different word corpus and
game parameters.  

We leave exploration of more advanced deep RL techniques to word games like
hangman to fugure work.
...
5.  [C. Moris, "Dissecting Google's Billion Word Language Model Part 1:
