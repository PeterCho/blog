...
within the TensorFlow framework [10].  We also download a subset of the
Open Images Dataset to serve as training, validation and testing data [11].
CNN training depends upon a sizable number of hyperparameters which must be
empirically set.  Table 2 summarizes the hyperparameter values that we
settled upon after running 30+ experiments.
