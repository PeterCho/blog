...
framework which is a learning system built on top of Caffe [7 - 9].
...
which we attempted to adjust using training and validation data was the
learning rate.  We observed relatively small changes in validation accuracy
metrics as we varied the learning rate parameter from 0.00005 to 0.016 and
trained on a subset of input image tiles.  The optimal value for the base
learning rate uncovered by these tests turned out to be 0.0005.  For all
other hyperparameters, we retained DeepLab defaults such as 0.9 momentum
and 0.0005 weight decay along with a step learning rate policy [8].
