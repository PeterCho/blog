...
Rather than employ conventional stochastic gradient descent to update
network weights, we instead utilize RMS propagation with a 0.9 decay rate
[13, 14].
...
The neural network's weights $\bf w$ are iteratively adjusted so as to
...
During network training, the loss function in eqn. (2) jumps around but
generally decreases over time.  The logarithmic plot in figure 4
...
*Fig. 7. Visualization of trained weights in the neural network's first
layer.*
...
13. [G. Hinton, "Neural Networks for Machine Learning", Slide 29 from
Lecture 6 Coursera notes,
((2012)).](https://www.coursera.org/learn/neural-networks)

14.  [A. Kaparthy, "Deep Reinforcement Learning: Pong from Pixels", ((2016)).](
