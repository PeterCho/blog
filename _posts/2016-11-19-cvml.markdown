...
We first represent a maze with $n \times n$ cells by a $(2n-1) \times
(2n-1)$ occupancy grid.  Grid locations with zero occupancy correspond to
...
are set equal to $-1 / n_{wall \; cells}$.  The sum over all occupancy grid
...
several experiments, we select a network configuration with two hidden
layers that each contain 10 nodes.  We adopt ReLU nonlinearities in all but
the final layers.  The last layer generates activations by an identity
operator.  During network training via conventional back propagation, we
initialize the learning rate to 0.0003 and slowly decrease it to 0.0001.
Following [hinton, kaparthy], we update network weights using RMS
propagation with a 0.9 decay rate.
...
alt="IMAGE ALT TEXT HERE" width="720" height="480" border="10" /></a>
*Fig. XX.  Movie illustrating deep Q learning for a 10x10 maze.  Click on
image above to play video.*


![MazeRL]({{site.url}}/blog/images/maze_rl/deepQ/trained_padded_weights.png)
*Fig. XX. Visualization of trained weights in the neural network's first
layer.*
...
13.  [T. Matiisen, "Demystifying Deep Reinforcement Learning", ((2016)).](https://www.nervanasys.com/demystifying-deep-reinforcement-learning/)

* Hinton coursera on RMS prop
