...
represents another perhaps impractical but pedagogically instructive RL
application.  In 2015, Deep Mind researchers developed a system which
achieved super-human performance on multiple Atari games [5].  Even more
impressively, another RL breakthrough made newspaper headlines when it
...
neighbor cell is assigned the direction vector pointing towards the current
cell.  Direction finding then recursively continues with the neighbor cell.
...
For our maze problem, lattice cells represent states, and up, right, down
or left single-cell movements correspond to actions.  If the agent performs
an illegal move which causes it to crash into an internal or external maze
wall, the agent immediately loses the game.  On the other hand, if the
agent successfully navigates its way through the maze to the bottom right
corner cell, it wins the game.  For RL systems learning to play games, it
is common that the reward function return zero for all moves except at
terminal states.  When the agent wins [loses] the game, we assign it a +1
[-1] reward.  After playing many game episodes, the agent should learn by
trial-and-error which set of states and actions lead to the winning end
state.

We initialize $Q(s,a)$ with random values ranging uniformly over the
interval [-1,1].  At the start of a new episode, we also randomly select
the agent's beginning location in the maze.  For every step in the game,
the agent's action is chosen randomly among up, right, left and down
movements.  Working with TD learning rate $\alpha= 0.75$ and reward
discount factor $\gamma = 0.95$, we then iteratively update the Q-values
using control equation ((1)).

As the solution to the $10 \times 10$ maze is already known, we can monitor
the progress of tabular Q-learning towards finding the answer displayed in
figure 1.  After each trial episode, we simply count the number of maze
...
The evolution of tabular Q-learning over time is portrayed in the movie of
figure 3.  Since $Q(s,a)$ is initially filled with noise, its maximal
action directions are randomly oriented at the movie's beginning.  Flow
field directions which cause the agent to illegally crash into an interior
or exterior maze wall are colored red.  Legal, though perhaps incorrect,
maze movements are indicated by arrows colored green.  Neighboring pairs of
arrows pointing in contradictory directions are colored orange.  As the
movie plays, we observe that the agent rapidly learns to not crash into
walls.  It takes longer for all contradictory direction pairs to be
resolved.  But eventually, reinforcement learning settles upon a
self-consistent maze solution which agrees with the result in figure 1.
...
figure 1.  Click on image above to play video.*
...
interesting applications of reinforcement learning have huge state spaces
which render tabular Q-learning totally impractical.  For such problems, we
must approximate $Q(s,a)$ via some function which can be learned by
trial-and-error.  Over the past decade, researchers have found a set of
tricks which enable neural networks to act as function approximators for
Q-learning [10-12].  We apply their techniques to maze solving.
...
problems, our network's output layer contains 4 nodes associated with up,
right, down and left movements.  The output nodes' values equal $Q(s,a)$.
It is important to note that no *a priori* constraint is placed upon
illegal actions for state $s$.  Instead, the agent must learn via negative
reinforcement to avoid illegal moves.

As for all other neural networks, we need to choose a set of
hyperparameters which govern how our maze network operates and trains.
After running several experiments, we select a fully-connected network
configuration with two hidden layers that each contain 10 nodes.  Following
ref. [13], we choose not to include bias terms into our network.  It
consequently contains 3750 variables whose values must be fit.
...
training via conventional back propagation, we initialize the network's
...
At the $n^{th}$ step in an episode, we choose action $a_n$ according to an
$\epsilon$-greedy policy [11,12].  At the start of network training,
$\epsilon = 1$.  But after every 1000 episodes, $\epsilon \rightarrow
\max(0.9 \, \epsilon, 0.01)$.  We also store up to the last $5 m^2$
transitions $(s_n, a_n, r_n, s'_{n+1})$ within a replay memory ${\cal D}$.
After every episode has finished, a random mini-batch of
...
The target term plays a role in reinforcement learning analogous to a class
label in supervised learning.  Researchers have previously found that using
frequently-updated weights in a Q-learning target often leads to neural
network training oscillations and instabilities [11, 12].  So they
recommend computing the target with a set of old, fixed ${\bf w^-}$
parameters and periodically updating ${\bf w^-}$.  We consequently copy
${\bf w} \to {\bf w^-}$ only after every tenth episode.
...
episodes (({\rm see \; figure \; 5})).  We note that the order of magnitude
increase in the number of training episodes needed to achieve convergence
for deep Q-learning compared to tabular Q-learning is comparable to the
increase in their ratio of free parameters.
...
The movie in figure 6 displays the evolution of the maze flow field from
random orientations to the self-consistent answer as deep Q-learning
progresses.  We again observe that the agent rapidly learns to not crash
into walls.  But it takes much longer for all contradictory pairs of
neighboring maze directions to be resolved.
...
It is instructive to examine trained weight values in order to gain insight
into what the neural network learns.  For visualization purposes only, we
...
Comparing the colored patterns for the trained weights in figure 7 with the
structure of the $10 \times 10$ maze in figure 6, we clearly see that the
network learns the wall layout for the maze.  While wall knowledge is an
obvious prerequisite for maze solving, it is reassuring to confirm that the
agent develops this basic understanding about its environment via
reinforcement learning.
...
a consistent pattern among larger trained weight values.  Perhaps more
insight could be gained by visualizing trained weight matrices linking
higher layers in the neural network.  We leave such exploration for future
work.
